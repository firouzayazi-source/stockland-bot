...
**۵) صفحهٔ خلاصه/ویرایش:** مرحلهٔ آخر قبل از `_iv_finalize` (که خودش کاملاً دست‌نخورده مونده). `_iv_ask_summary` همهٔ پاسخ‌ها رو با یه دکمهٔ ✏️ به‌ازای هر بخش (`ivw_edit_{step_id}`) نشون می‌ده. زدن ✏️ یعنی `state["editing"]=step_id` و `_iv_goto(chat_id, uid, step_id)`؛ `_iv_step_done` تا وقتی داخل همون «گروه» مرحله‌ای بمونیم (مثلاً ویرایش مدل باید ظرفیت/رنگ/پارت وابسته رو هم دوباره بپرسه) ادامهٔ خطی می‌ره، ولی همین که گروه عوض شد، بلافاصله برمی‌گرده به خلاصه (نه ادامهٔ کل زنجیره). `ivw_summary_confirm`→`_iv_finalize`، `ivw_summary_cancel`→پاک‌کردن state.

**۶) پنل ادمین:** دو صفحهٔ تازه زیر همون permission `ai_pricing`: `/admin/iphone/series` (بخش ۱ بالا) و `/admin/iphone/repairs` («مدیریت تعمیرات» — طبق درخواست صریح مالک پروژه یک لیست/فرم واحد با دو درصد کنار هم برای هر قطعه، نه دو کارت جدا مثل بقیهٔ دسته‌های `/iphone/coefficients`). صفحهٔ عمومی `/iphone/coefficients` دیگه `component`/`replaced` رو نشون نمی‌ده.
