...
**`/admin/iphone/models` (بازطراحی‌شده، ۲۰۲۶-۰۷-۲۵، دور سوم برای کلید روشن/خاموش):** هر مدل یه `<details>` جدا. فرم ویرایش نام/سری دو چک‌باکس هم داره: «رنگ روی قیمت اثر داره» (همیشه نشون داده می‌شه) و «پارت روی قیمت اثر داره» (فقط اگه مدل پارت‌محور باشه). ظرفیت‌ها زیر یک هدر گروه‌بندی‌شده بر اساس برچسب ظرفیت نمایش داده می‌شن؛ هر ردیف قیمت بج پارت (اگه مدل پارت‌محور باشه) + بج رنگ (همیشه) + دکمهٔ حذف داره. فرم واحد «📝 ثبت/به‌روزرسانی قیمت این مدل»: دراپ‌داون ظرفیت (`ivdb.list_capacity_labels(model_id)` + «سایر» دستی) همیشه هست؛ **دراپ‌داون رنگ فقط وقتی `color_pricing` روشن باشه ظاهر می‌شه**؛ **دراپ‌داون پارت فقط وقتی مدل پارت‌محور باشه *و* `part_pricing` روشن باشه** — وگرنه اون دراپ‌داون کلاً از فرم حذف می‌شه (نه فقط مخفی/غیرفعال)، بعدش باکس‌های قیمت، دکمهٔ ذخیره که `POST /iphone/capacities/upsert` رو صدا می‌زنه (این route هم سمت سرور دوباره طبق همین دو فلگ رنگ/پارت رو صفر می‌کنه، حتی اگه فرم دستکاری شده باشه — دفاع در عمق). فرم «+ افزودن رنگ» (مدیریت کاتالوگ رنگ‌های این مدل، منبع دراپ‌داون بالا) جدا مونده، مستقل از فلگ `color_pricing` (چون رنگ همیشه برای نمایش به کاربر لازمه، صرف‌نظر از اثرش روی قیمت). همهٔ فیلدهای قیمت `type="text" inputmode="numeric/decimal"` هستن (نه `type="number"` — بخش ۱۷ رو ببین، دلیلش رو). دکمهٔ «حذف مدل» با `confirm()` کنار toggle فعال/غیرفعال.
