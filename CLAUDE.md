...
> تاریخ آخرین بروزرسانی افزایشی: ۲۰۲۶-۰۷-۲۵ — بعد از چند راند توسعهٔ مینی‌اپ (بخش ۲۱) + افزودن فیچر مستقل «کارشناس هوشمند قیمت آیفون» (بخش ۲۲) — قبل از کار روی هرکدوم، همون بخش رو بخون تا نیازی به خوندن دوبارهٔ کد نباشه.
...
- افزودن `app_content`, `daily_checkins`, `favorites`, `user_notifications`, و همهٔ جداول `iv_*` به ماژول‌های `stbak_engine.py`
...
`iv_models` (مدل+سری+`dual_sim_parts`+`esim_only`)، `iv_capacities` (ظرفیت+**`part_number`**+قیمت‌پایه+قیمت‌مرجع‌خرید+قیمت‌مرجع‌فروش+نرخ‌ارز‌مرجع+عرضه‌تقاضا — هر ردیف یه ترکیب مدل+ظرفیت+پارت)، `iv_colors`، `iv_coefficients` (دسته+کلید+برچسب+درصد، هشت دسته شرایط دستگاه شامل `component`)، `iv_score_weights` (وزن هر دسته برای امتیازدهی)، `iv_fx_sources` (منابع نرخ ارز)، `iv_transactions` (تاریخچهٔ معاملات واقعی — فعلاً بدون UI ثبت)، `iv_valuations` (لاگ کامل هر کارشناسی، برای تاریخچه/آمار پنل).

### قیمت‌گذاری پارت‌محور (از ۲۰۲۶-۰۷-۲۵)
قیمت هر گوشی بسته به پارت نامبر (LL/A, ZA/A, CH/A) می‌تونه فرق کنه، پس `iv_capacities` دیگه یکتا-به-ازای-(مدل+ظرفیت) نیست — یکتا-به-ازای-(مدل+ظرفیت+`part_number`) هست؛ `part_number=''` یعنی «قیمت عمومی بدون پارت مشخص». `ivdb.resolve_capacity(model_id, capacity_label, part_number)` قیمت رو پیدا می‌کنه: اول دنبال ردیف دقیق همون پارت می‌گرده، اگه نبود به ردیف عمومی (`part_number=''`) همون ظرفیت fallback می‌کنه — یعنی ادمین فقط جایی که قیمت واقعاً فرق داره لازمه پارت‌به‌پارت وارد کنه، بقیه از قیمت عمومی استفاده می‌کنن. `ivdb.capacity_exists()` قبل از افزودن چک می‌کنه یه (مدل، ظرفیت، پارت) دوبار ثبت نشه. `ivdb.list_capacity_labels(model_id)` برچسب‌های یکتای ظرفیت (صرف‌نظر از پارت، مرتب‌شده با `capacity_sort_key` به ترتیب اندازهٔ واقعی نه الفبایی) رو برمی‌گردونه — برای مرحلهٔ انتخاب ظرفیت در ویزارد ربات.

**پارت فقط برای مدل‌های `dual_sim_parts`-دار پرسیده/پیکربندی می‌شه** (از iPhone XS Max به بعد؛ iPhone XS و پایین‌تر چون `dual_sim_parts` خالیه، اصلاً پرسیده نمی‌شه — همون فلگ سیاست سیم‌کارت موجود `_iv_sim_policy` که از قبل به‌درستی iPhone XS رو با `sort_order<20` تک‌سیم حساب می‌کرد، بدون نیاز به تغییر).
...
داشبورد+آمار → `/admin/iphone/models` (CRUD مدل/ظرفیت/قیمت/رنگ + حذف) → `/admin/iphone/coefficients` (CRUD ضرایب هر دسته + وزن امتیازدهی) → `/admin/iphone/fx` (منابع نرخ ارز + حالت دستی/خودکار + حساسیت + وزن دادهٔ بازار) → `/admin/iphone/history` (لاگ کامل کارشناسی‌ها). فعال/غیرفعال‌سازی کل قابلیت با دکمهٔ توگل در داشبورد، که چیزی جز `set_main_button_enabled("MAIN_BTN_IPHONE_VALUATION", ...)` نیست — همون مکانیزم `MAIN_BUTTON_KEYS` استاندارد پروژه (بخش ۱۶، قانون ۴)، نه مکانیزم جدید.

**`/admin/iphone/models` (بازطراحی‌شده، ۲۰۲۶-۰۷-۲۵):** هر مدل یه `<details>` جدا؛ ظرفیت‌ها زیر یک هدر گروه‌بندی‌شده بر اساس برچسب ظرفیت (نه پارت) نمایش داده می‌شن — برای مدل‌های پارت‌محور (`dual_sim_parts` پر)، هر پارت یه ردیف قیمت جدا با بج پارت + دکمهٔ حذف داره، به‌علاوه یه فرم کوچیک «+ قیمت پارت دیگه» که فقط پارت‌های هنوز-ثبت‌نشده رو پیشنهاد می‌ده. فرم «+ افزودن ظرفیت جدید» بالای هر مدل: دراپ‌داون حجم استاندارد (`ivdb.STANDARD_CAPACITIES`) + گزینهٔ «سایر» با فیلد دستی، به‌علاوه دراپ‌داون پارت (فقط اگه مدل پارت‌محور باشه). فرم «+ افزودن رنگ» هم به همین سبک (دراپ‌داون رنگ‌های رایج `_IV_COMMON_COLORS` + «سایر»). همهٔ فیلدهای قیمت `type="text" inputmode="numeric/decimal"` هستن (نه `type="number"` — بخش ۱۷ رو ببین، دلیلش رو). دکمهٔ «حذف مدل» با `confirm()` کنار toggle فعال/غیرفعال.

### ربات (`bot.py`)
دکمهٔ `MAIN_BTN_IPHONE_VALUATION` در منوی اصلی (`keyboards.main_menu`). ویزارد اینلاین با `user_states[uid]["mode"]="iphone_valuation"` → `"iv_seller_price"` → `"iv_city"` (سه حالت متنی/اینلاین ترکیبی). مراحل (ترتیب از ۲۰۲۶-۰۷-۲۵ عوض شد): انتخاب مدل → **[فقط اگه `dual_sim_parts` پر باشه] پارت نامبر** → ظرفیت (`ivdb.list_capacity_labels`، برچسب‌های یکتا صرف‌نظر از پارت) → با انتخاب ظرفیت، `ivdb.resolve_capacity(model_id, label, part_number)` قیمتِ درستِ همون ترکیب رو پیدا می‌کنه (`state["capacity_id"]`) → هشت گزینهٔ شرایط دستگاه (از `_IV_COEFF_STEPS`، به ترتیب `iv_coefficients`) → تست امکانات (بله/خیر) → نوع فروشنده → قیمت پیشنهادی (متن آزاد، «ندارم» = رد شدن) → شهر (متن آزاد، «ندارم» = رد شدن) → `_iv_finalize` که `service.valuate()` رو صدا می‌زنه و پیام نهایی رو می‌سازه (برچسب پارت در پیام نهایی از `ivdb.PART_OPTIONS` می‌گیره، نه کد خام).

**چرا ترتیب عوض شد:** قیمت نهایی به هم‌زمان ظرفیت *و* پارت وابسته‌ست (`resolve_capacity` هردو رو می‌خواد)، پس پارت باید قبل از قطعی‌شدن انتخاب ظرفیت مشخص بشه. برای مدل‌هایی که `dual_sim_parts` خالیه (iPhone XS و پایین‌تر، مینی‌ها) این سؤال کلاً پرسیده نمی‌شه چون بی‌معنیه — `state["part_number"]=""` می‌مونه و `resolve_capacity` مستقیم سراغ ردیف عمومی می‌ره.
