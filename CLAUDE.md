...
> تاریخ آخرین بروزرسانی افزایشی: ۲۰۲۶-۰۸-۰۷ — رفع ۳ باگ محاسباتی حسابداری + ادغام گزارش مالی + چپ‌چین اعداد پنل، رفع باگ سراسری دکمهٔ نامرئی (متغیر CSS `--clr-primary`)، بازطراحی صفحهٔ خلاصهٔ ویزارد کارشناسی آیفون، تکمیل حسابداری با Import عمومی TXT/CSV/Excel + «تعویض کالا» + گزارش‌گیری، رفع نوتیف درخواست فروش + راست‌چین/چپ‌چین ستاره‌ها-نمودارها + کیف‌پول پاداش مینی‌اپ، بازطراحی Premium صفحهٔ «دعوت دوستان» مینی‌اپ، رفع کامل متن/اشتراک‌گذاری دعوت (تگ HTML خام، تکرار لینک، برگشت به الگوی استاندارد `url=+text=` تلگرام)، مسیر اختصاصی آپلود اکسل موجودی محصول چندستونی + چپ‌چین apple id در پنل + توضیحات مشترک اختیاری برای آپلود موجودی، رفع رنگ نامرئی حباب پیام ادمین چت تیکت (تکرار باگ `--clr-primary`) + رفع تغییر نام/پسوند فایل پیوست تیکت پس از انتقال به پنل + امکان ارسال عکس/فایل از پنل به کاربر در چت تیکت + تکمیل سیستم برگشت محصول (رفع باگ نمایش سفارش برگشتی در خریدهای من + بستن خودکار تیکت راه‌اندازی وابسته) + پاکسازی خودکار کش و فایل‌های موقت + یکسان‌سازی پذیرش رقم فارسی/عربی/لاتین در ورودی مبلغ (بات+مینی‌اپ) + تکمیل واقعی سیستم پاداش/چرخهٔ معرفی برای کاربران غیرهمکار (رفع باگی که تحلیل قبلی روی تابع مردهٔ اشتباه انجام داده بود) + رفع ریشه‌ای سیستم برگشت محصول (بازگشت به موجودی بی‌اثر در اکثریت سفارش‌ها + پیام تحویل غیرقابل‌حذف از چت، هر سه مسیر تحویل بات/درگاه/مینی‌اپ) + استاندارد وضعیت پرداخت برای تحویل‌های اصلاحی/تعویضی + ادغام کامل صفحات ویرایش/برگشت/تعویض/ارسال‌مجدد سفارش به «وضعیت سفارش» یکپارچه + رفع باگ ۴۰۴ دکمهٔ انصراف + گزینهٔ کیف‌پول در ارسال مجدد + لاگ فعالیت سفارش‌ها + بازطراحی دوم «وضعیت سفارش» (حذف کارت اطلاعات/تاریخچهٔ تکراری، ادغام تعویض+ارسال‌جایگزین در یک تب هوشمند، لاگ فعالیت فقط یک‌جا با نمایش کشویی/اسکرول‌دار) + حذف کامل کارت ویرایش اطلاعات پایه از همون صفحه + ممیزی سریع کل پروژه با رفع ۶ مورد امنیتی (بای‌پس یوزرنیم، هش پسورد ضعیف، دو پیش‌فرض SESSION_SECRET، رمز plaintext در .env، بدون CSRF، جعل هویت API_KEYS) و ۵ مورد کارایی (ایندکس‌های گمشدهٔ برگشت/تعویض سفارش، DDL تکراری لاگ فعالیت)، رفع باگ `add_product`/مهاجرت `chat_enabled` + **تکمیل واقعی مهاجرت Postgres** (بخش ۴۶ — کشف شد که بخش بزرگی از اپ از جمله کل `admin_panel._db()` مستقل از `DB_DIALECT` مستقیم SQLite می‌زد؛ رفع مسیریابی اتصال در ۳۷ نقطه + ۳ باگ جدید کشف‌شده با تست مستقیم روی Postgres واقعی نصب‌شده در سندباکس — transaction poisoning هر `ALTER TABLE ADD COLUMN` تکراری، `RealDictCursor` بدون دسترسی اندیس عددی، `lastrowid` همیشه `None` — + `INSERT OR REPLACE`→`ON CONFLICT` در ۹ نقطه + سیستم بکاپ کاملاً غیرمرتبط با دادهٔ واقعی زیر Postgres، رفع با فعال‌سازی `pg_backup.py`) + **تکمیل مهاجرت (دور دوم)**: رفع باگ‌های واقعی باقی‌مانده در بخش «کاربران»/«کارشناسی آیفون» گزارش‌شده توسط مالک پروژه — پیدا شده با هارنس crawler (Postgres واقعی + TestClient روی هر ۶۵ مسیر پنل) نه حدس: CAST اشتباه در جوین سفارش‌های کاربر، چند مورد GROUP BY strictness (`/feed`, `/iphone`, حسابداری)، و مهم‌ترین مورد — الگوی `col IS ?` (مقایسهٔ NULL-safe) که موتور قیمت‌گذاری آیفون رو کاملاً از کار انداخته بود روی Postgres (رفع در سطح `db_dialect.py`)، به‌علاوهٔ یه باگ preexisting و دیالوگ‌مستقل (ترتیب ثبت route که خروجی اکسل کاربران/صفحهٔ محصولات تکراری رو همیشه ۴۲۲ می‌داد) + بازطراحی کامل صفحهٔ «پشتیبان‌گیری و بازیابی» (بخش ۴۸) + رفع بازیابی بکاپ‌های قدیمی ZIP-محور روی Postgres با تشخیص خودکار فرمت + خودترمیمی نوع ستون (بخش ۴۹) + رفع fallback خاموش به SQLite در `db.py._get_connection()` + ممیزی نهایی مسیرهای مشتری‌محور قبل از معرفی ربات به کاربر واقعی (بخش ۵۰) + **رفع بحرانی: `BEGIN IMMEDIATE` کل مسیر خرید رو روی Postgres از کار انداخته بود** (بخش ۵۱ — سینتکس اختصاصی SQLite که Postgres نمی‌شناسه، هیچ خریدی کار نمی‌کرد؛ رفع دوسطحی در `db_dialect.py` + قفل ردیف واقعی `FOR UPDATE` روی ۵ تابع اتمیک بحرانی تا تضمین همزمانی SQLite حفظ بشه، تست‌شده با ۲۰ ترد هم‌زمان روی Postgres واقعی) + ممیزی جامع نهایی قبل از معرفی به مشتری واقعی (بخش ۵۲ — رفع `rowid` خاص SQLite در پاک‌سازی موجودی + ۶ نقطهٔ `LIKE` حساس به بزرگ/کوچیک روی Postgres در جست‌وجوهای محصول/کاربر/سفارش/لاگ) (بخش ۵۲) + انتقال مهاجرت lazy کارشناسی آیفون (`iphone_valuation.db.ensure_schema()`) به startup — رفع ۵۰۰ تصادفی روی اولین کلیک بعد از هر ری‌استارت سرویس (بخش ۵۳) + **گردونهٔ شانس (Wheel of Fortune) کامل Production-Grade** (بخش ۵۴ — سیستم پاداش عمومی مینی‌اپ با گسترش موتور `discount_codes` موجود (نه سیستم موازی)؛ کمپین/جایزه/تنظیمات کاملاً از پنل بدون Hard Code؛ انتخاب جایزه ۱۰۰٪ سمت سرور با قفل اتمیک ضدتقلب (تست‌شده با ۲۰ چرخش هم‌زمان روی Postgres واقعی)؛ حین ساخت یه باگ preexisting واقعی هم پیدا/رفع شد — مقایسهٔ فرمت نادرست تاریخ در `validate_discount` که هر کد تخفیف با انقضای همون‌روزه (از جمله winback) رو کاذب «منقضی‌شده» نشون می‌داد؛ UI مینی‌اپ چرخ‌گردون Premium با conic-gradient/کانفتی/صدا/لرزش، همه بدون وابستگی خارجی) (بخش ۲۹ تا ۵۴) — قبل از کار روی هرکدوم، همون بخش رو بخون تا نیازی به خوندن دوبارهٔ کد نباشه.
...
- **دیتابیس تولید:** PostgreSQL (`DB_DIALECT=postgres` + `DATABASE_URL` در `.env`) — ⚠️ تا ۲۰۲۶-۰۸-۰۷ این سند اشتباهاً SQLite رو دیتابیس تولید معرفی می‌کرد؛ مالک پروژه مدتی قبل مهاجرت کرده بود ولی خیلی از کد اصلاً از لایهٔ Postgres عبور نمی‌کرد (بخش ۴۶). `DB_PATH` هنوز env اجباریه (بدون آن `RuntimeError`) چون بخشی از کد (بکاپ/ANALYZE قدیمی) هنوز بهش رفرنس می‌ده، ولی دیگه مسیر دادهٔ اصلی نیست. حالت SQLite (`DB_DIALECT` تنظیم‌نشده/`sqlite`) هم برای dev/تست محلی همچنان کاملاً پشتیبانی می‌شه.
...
دیتابیس تولید PostgreSQL است (بخش ۴۶ — قبلاً SQLite بود، مهاجرت کامل شد)؛ حالت SQLite هم برای dev/تست محلی کاملاً پشتیبانی می‌شه (بدون تنظیم `DB_DIALECT`). بیش از ۴۰ جدول در `db.py` تعریف شده‌اند (+ چند جدول در `bot.py`/`admin_panel.py`/`payment_service.py`).
...
| `DB_DIALECT`, `DATABASE_URL` | سوییچ SQLite↔Postgres — تولید `DB_DIALECT=postgres` (بخش ۴۶). بدون تنظیم، پیش‌فرض `sqlite` (dev/تست محلی) |
...
2. ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷، بخش ۴۵)** — ~~رمز سوپرادمین plaintext روی دیسک~~ — `POST /admin/admins/super/password` حالا هش نمکی PBKDF2 (`_hash_pw`) رو در `.env` می‌نویسه، نه رمز خام. `_verify_super_pw` هم فرمت جدید هم plaintext قدیمی (برای مقدار bootstrap دستی) رو می‌پذیره. `/admins/super/telegram_id` (که فقط `ADMIN_ID` عددی، نه راز، می‌نویسه) عمداً دست‌نخورده موند.
3. ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷، بخش ۴۵)** — ~~یوزرنیم‌های bypass هاردکد~~ — `"admin"`/`"super"` فقط وقتی به‌عنوان نام مستعار پذیرفته می‌شن که هیچ `ADMIN_WEB_USERNAME` سفارشی در env تنظیم نشده باشه؛ به‌محض تنظیم یوزرنیم سفارشی، فقط خودش معتبره.
4. ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷، بخش ۴۵)** — ~~دو پیش‌فرض متفاوت برای `SESSION_SECRET`~~ — یکپارچه شد (`_SESSION_SECRET_DEFAULT = "stockland-panel"`، همه‌جا). پیش‌فرض قدیمی `"stockland"` فقط برای وریفای هش‌های legacy نگه داشته شده (بند ۵).
5. ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷، بخش ۴۵)** — ~~هش پسورد ادمین‌ها ضعیف~~ — از `SHA256(SESSION_SECRET+password)` بدون نمک به PBKDF2-HMAC-SHA256 نمکی (۲۰۰هزار تکرار، فرمت `pbkdf2$salt$hash`) تغییر کرد. سازگاری عقب‌رو کامل: `_verify_pw` هر دو فرمت رو می‌پذیره و بعد از اولین ورود موفق با هش قدیمی، خودکار به فرمت جدید ارتقا می‌ده (بدون نیاز به اقدام دستی).
6. ✅ **رفع‌شده جزئی (۲۰۲۶-۰۸-۰۷، بخش ۴۵)** — ~~بدون CSRF token~~ — به‌جای دستکاری هزاران خط فرم HTML موجود، یک میدل‌ور دفاع-در-عمق (`_csrf_guard`، `payment_service.py`) اضافه شد: هر POST/PUT/PATCH/DELETE به `/admin/*` باید هدر Origin/Referer هم‌خوان با Host داشته باشه، وگرنه ۴۰۳. این جایگزین کامل CSRF token واقعی نیست (لایهٔ دوم OWASP-recommended)؛ افزودن token واقعی per-form هنوز کاری پرریسک/بزرگ برای فاز بعده.
...
8. ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷، بخش ۴۵)** — ~~`API_KEYS`/`X-User-Id` بدون امضا~~ — `api.py:_auth` حالا علاوه بر `X-API-Key`، هدر `X-Internal-Secret` (باید دقیقاً `INTERNAL_API_SECRET` باشه — همون رازی که از قبل `/payment/create` رو محافظت می‌کنه، بند ۱۱) رو هم اجباری می‌کنه. بررسی شد هیچ کلاینت داخلی فعلی از این مسیر استفاده نمی‌کنه — پس هیچ caller مشروعی نشکست.
...
| 3 | ~~دکمهٔ ادمین «فعال‌سازی چت محصول» همیشه بی‌اثر است~~ | `db.py`, `bot.py` | ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷)** — ستون مهاجرت شد + `db.get/set_product_chat_enabled()` جایگزین اتصال موازی قدیمی در `bot.py` شد |
...
| 6 | ~~`db.add_product()` متغیر `cols` تعریف می‌شود ولی هیچ‌جا پر نمی‌شود~~ | `db.py: add_product` | ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷)** — حالا واقعاً از `PRAGMA table_info(products)` پر می‌شه |
...
- ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷):** افزودن مهاجرت `products.chat_enabled`
...
- ✅ **رفع‌شده (۲۰۲۶-۰۸-۰۷):** رفع باگ `db.add_product()` (متغیر `cols` بلااستفاده، `product_key` هیچ‌وقت درج نمی‌شود)
...
| گردونهٔ شانس (چرخش روزانه، جایزهٔ سمت‌سرور) + «جوایز من» — تفصیل کامل بخش ۵۴ | `GET /wheel/state`, `POST /wheel/spin`, `GET /me/prizes` | `wheel_campaigns`/`wheel_prizes`/`wheel_spins`/`wheel_daily_usage` + `discount_codes` گسترش‌یافته | `/admin/wheel/*` (تنظیمات/کمپین‌ها/جوایز/تاریخچه/داشبورد) |
...
## ۴۲. ادغام کامل صفحات مدیریت سفارش در «وضعیت سفارش» + رفع ۴۰۴ انصراف + کیف‌پول در ارسال مجدد + لاگ فعالیت (۲۰۲۶-۰۸-۰۶، ادامهٔ همون روز)

مالک پروژه چهار مشکل مرتبط با بخش ۴۱ گزارش داد: (۱) دکمهٔ «انصراف» در صفحهٔ تعویض کالا ۴۰۴ می‌داد؛ (۲) دکمهٔ «ویرایش» سفارش‌ها به‌جای رفتن به یه صفحهٔ مدیریت کامل، فقط فرم ویرایش عنوان/قیمت رو نشون می‌داد — درخواست این بود که ویرایش/برگشت/تعویض/ارسال‌مجدد همه یک صفحهٔ یکپارچه بشن، با تغییر نام از «برگشت سفارش» به «وضعیت سفارش»؛ (۳) پایین صفحهٔ لیست سفارش‌ها یه لاگ فعالیت حرفه‌ای (چه محصولی با چی عوض شد، دلیل برگشت، خلاصهٔ هر اتفاق) نمایش داده بشه؛ (۴) ارسال محصول جایگزین (دکمهٔ «ارسال مجدد») باید مثل فرم برگشت، گزینهٔ کسر/افزایش/بدون‌تغییر کیف‌پول داشته باشه.

قبل از کدنویسی، طبق درخواست صریح («قبل از اینکه کد بزنی بهم بگو فهمیدی چی می‌گم»)، پلن به فارسی تأیید شد: یک صفحهٔ هاب (`GET /admin/orders/{oid}`) با کارت‌های جدا (اطلاعات، ویرایش، برگشت، تعویض، ارسال‌مجدد، تاریخچه) به‌جای ۴ صفحهٔ مجزا؛ لاگ از جدول موجود `admin_logs` (نه جدول تازه) هم به‌صورت خلاصهٔ سراسری (پایین لیست سفارش‌ها) هم تاریخچهٔ اختصاصی هر سفارش (داخل هاب).

### ریشهٔ باگ ۴۰۴ «انصراف»
دکمه‌های «انصراف» صفحات قدیمی برگشت/تعویض به `/admin/orders/{oid}` (بدون هیچ زیرمسیر) لینک می‌دادن — مسیری که هیچ‌وقت به‌عنوان یه route ثبت نشده بود (فقط `/edit`, `/return`, `/exchange`, `/resend` وجود داشتن). همین لینک شکسته توی ۴ نقطه تکرار شده بود: دو دکمهٔ انصراف، یه ریدایرکت گارد داخل `order_exchange_form` قدیمی، و لینک «سفارش جدید #X» در گزارش `/admin/accounting/exchanges`. ساخت هاب دقیقاً روی همین آدرس (`/admin/orders/{oid}`) این مشکل رو ریشه‌ای رفع کرد — دیگه نیازی به یه دکمهٔ «انصراف» جدا هم نبود، چون فرم‌ها دیگه صفحهٔ مستقل نیستن، کارت‌های همون صفحه‌ان.

### پیاده‌سازی
- **حذف کامل** ۴ صفحهٔ GET مجزا: `order_edit_get` (`/orders/{oid}/edit`)، `order_return_form` (`/orders/{oid}/return`)، `order_resend_page` (`/orders/{oid}/resend`)، `order_exchange_form` (`/orders/{oid}/exchange`) — منطق POST هرکدوم (`order_edit_post`, `order_return`, `order_resend_post`, `order_exchange_post`) دست‌نخورده موند، فقط همهٔ ریدایرکت‌های خطا/موفقیتشون به هاب تازه (`/admin/orders/{oid}`) اصلاح شد (تعویض موفق به هاب سفارش **جدید** ریدایرکت می‌شه، نه لیست).
- **صفحهٔ تازهٔ `GET /admin/orders/{oid}` (`order_status_page`)** — عنوان «وضعیت سفارش #{oid}»، شامل: کارت اطلاعات سفارش (با همون بج‌های `payment_note` بخش ۴۱)، کارت ویرایش (همیشه)، کارت برگشت + کارت تعویض (فقط اگه `status='active'`)، کارت ارسال‌مجدد (همیشه)، کارت تاریخچهٔ همین سفارش. فیلدهای فرم‌ها عیناً با نام‌های قدیمی نگه داشته شدن تا POST handlerها بدون تغییر کار کنن.
- **گزینهٔ کیف‌پول در ارسال مجدد:** `order_resend_post` حالا `wallet_action` (`none`/`custom_add`/`custom_deduct`) + `custom_amount` می‌گیره — دقیقاً الگوی فرم برگشت (بدون گزینهٔ `full`، چون ارسال مجدد یه بازگشت کامل سفارش نیست). موجودی کیف‌پول با `max(0, ...)` تنظیم می‌شه؛ اگه ردیف کیف‌پول کاربر نبود، ساخته می‌شه. پیام نوتیف تلگرام کاربر هم مبلغ افزوده/کسرشده رو گزارش می‌ده.
- **لاگ فعالیت سفارش‌ها** — دو تابع تازه: `_fetch_order_logs(conn, oid=None, limit=20)` (کوئری روی `admin_logs WHERE section='سفارش‌ها'`، با `oid=None` یعنی سراسری/۱۵ مورد آخر، با `oid` مشخص یعنی فقط لاگ‌های همون سفارش با matching روی الگوی `"سفارش #{oid}"` در ستون `details`) و `_order_log_row_html(row)` (رندر یک ردیف با آیکون بر اساس نوع action، از `_ORDER_LOG_ICONS`). لیست سفارش‌ها (`orders_list`) پایین صفحه یه کارت «🕓 آخرین فعالیت‌های سفارش‌ها» گرفت؛ هاب هر سفارش هم کارت تاریخچهٔ اختصاصی خودش رو داره.
- **یکی‌سازی دکمهٔ لیست سفارش‌ها:** سه دکمهٔ قبلی (ویرایش/برگشت/تعویض) به یک دکمهٔ «🧾 وضعیت سفارش» تبدیل شد که برای همهٔ سفارش‌ها (فعال یا برگشتی) یکسان به هاب می‌ره.
- **رفع دو لینک باقی‌ماندهٔ داشبورد** («سفارش‌های اخیر») که به مسیر حذف‌شدهٔ `/orders/{oid}/edit` اشاره می‌کردن — به `/admin/orders/{oid}` (هاب) اصلاح شدن.

### باگ جانبی کشف‌شده حین تست — `_log()` هیچ‌وقت جدول `admin_logs` رو خودش نمی‌ساخت
حین ساخت تست end-to-end، مشخص شد `_fetch_order_logs` روی دیتابیس تازه با `OperationalError: no such table: admin_logs` شکست می‌خوره. علت ریشه‌ای عمیق‌تر از این تابع تازه بود: خودِ `_log()` (تابع مرکزی لاگ‌گیری پنل، از قبل در پروژه) هیچ‌وقت `CREATE TABLE IF NOT EXISTS admin_logs` نداشت — فقط یه `ALTER TABLE ADD COLUMN result` تدافعی (که خودش هم روی جدول ناموجود شکست می‌خورد) و بعدش مستقیم `INSERT`. تا این نشست، این جدول فقط وقتی واقعاً ساخته می‌شد که یکی از ۳ روت خاصی که `_ensure_theme_table()` رو صدا می‌زدن، قبلاً اجرا شده باشن — یعنی روی یه نصب کاملاً تازه که هنوز هیچ ادمینی وارد `/admin/settings/theme` یا مسیرهای مشابه نشده، **اولین فراخوانی `_log()` بی‌صدا شکست می‌خورد** (قورت داده‌شده توسط `except Exception: pass` خودِ `_log()`) و اون لاگ برای همیشه از دست می‌رفت — بدون هیچ خطای قابل‌مشاهده‌ای، دقیقاً همون کلاس باگی که بخش ۷ سند (الگوی مهاجرت) هشدار می‌ده. رفع: `CREATE TABLE IF NOT EXISTS admin_logs (...)` مستقیم داخل خودِ `_log()`، قبل از `ALTER`/`INSERT` — الان `_log()` روی هر دیتابیسی، حتی کاملاً تازه، از همون اولین صدازدن درست کار می‌کنه.

**تست انجام‌شده:** هارنس مستقیم روی دیتابیس کاملاً تازه (بدون هیچ مهاجرت لیزی از قبل اجراشده) — سناریوی کامل نه‌مرحله‌ای: ویرایش سفارش (تأیید ریدایرکت به هاب + ثبت لاگ)، رندر هاب فعال (وجود هر ۴ کارت + تاریخچه شامل لاگ ویرایش)، برگشت با `custom_deduct` (تأیید کسر دقیق کیف‌پول ۱۰۰,۰۰۰→۹۷,۰۰۰ + بازگشت واقعی به موجودی)، رندر هاب برگشتی (کارت برگشت/تعویض مخفی)، ارسال مجدد با `custom_add` (تأیید افزایش کیف‌پول ۹۷,۰۰۰→۹۸,۵۰۰ + claim صحیح آیتم فید + `resent_at` ست‌شده + سفارش دوباره `active`)، رندر هاب بعد از ارسال مجدد، رندر لیست سفارش‌ها (دکمهٔ یکپارچه + بخش لاگ سراسری)، تعویض کامل (سفارش قدیم `returned`، سفارش جدید `active`، لینک دوطرفهٔ `exchange_pair_id` صحیح)، و تأیید نبود لینک ۴۰۴ (مسیر هاب برای سفارش تازه‌ساخته‌شده هم بدون خطا رندر می‌شه). `python3.12 -m py_compile admin_panel.py` بدون خطا.

فایل‌های تغییرکرده: `admin_panel.py`.

---

## ۴۳. بازطراحی دوم «وضعیت سفارش» — حذف کارت‌های تکراری + ادغام هوشمند تعویض/ارسال‌جایگزین + لاگ فعالیت کشویی (۲۰۲۶-۰۸-۰۷)

بعد از تأیید ادغام بخش ۴۲، مالک پروژه یه دور دوم اصلاح خواست، با «اختیار تام برای بهینه کردن UX/UI و دقت در کار»:

1. **حذف کامل کارت «اطلاعات سفارش»** از صفحهٔ «وضعیت سفارش» — این اطلاعات (شماره/محصول/کاربر/مبلغ/موجودی/تاریخ) از قبل توی ردیف لیست سفارش‌ها دیده می‌شن، تکرارشون توی هاب معنی نداشت.
2. **ستون «وضعیت» در لیست سفارش‌ها فقط بج خام** (فعال/برگشتی) نشون بده — توضیح اضافه (بج‌های «اصلاح‌شده — بدون پرداخت اضافی» / «از تعویض سفارش #X» که در بخش ۴۱ اضافه شده بودن) از اونجا حذف بشه، چون این جزئیات قراره فقط توی لاگ فعالیت (بند ۴) مدیریت بشه.
3. **ادغام «تعویض کالا» و «ارسال کالای جایگزین» در یک تب واحد** — هدف مشترک هر دو، دادن یه آیتم متفاوت به کاربره (یا از همون محصول، یا محصولی کاملاً دیگه)؛ هر قابلیت تکراری بین این دو حذف بشه.
4. **لاگ فعالیت فقط یک‌جا** (پایین صفحهٔ لیست سفارش‌ها) — بهتر/خواناتر برای ادمین، در یک بخش کشویی که ~۵ ردیف آخر قابل‌دید باشن و بقیه اسکرول بشن؛ دیگه جای دیگه‌ای (از جمله داخل هاب «وضعیت سفارش») نباید تکرار بشه.

### پیاده‌سازی

**هاب «وضعیت سفارش» (`order_status_page`):** `info_card` و `history_card` کاملاً حذف شدن. تنها چیزی که بالای صفحه می‌مونه یه بج وضعیت خام (فعال/برگشتی، بدون توضیح اضافه) کنار عنوانه. کارت «ویرایش» دست‌نخورده موند (چون این یه قابلیت عملیاتیه، نه نمایش خام — فیلدهای عنوان/قیمت خودشون به‌عنوان مرجع اطلاعات کافی‌ان). کارت‌های برگشت/تعویض/ارسال‌مجدد قبلی به یک کارت دوتبی («↩️ برگشت» / «🔄 تعویض · ارسال جایگزین») تبدیل شدن (JS سبک `stlOrderTab()`، بدون کتابخونهٔ اضافه، همون سبک بقیهٔ پنل).

**ادغام تب دوم (تعویض/جایگزین) — منطق شرطی، نه فقط چیدمان بصری:**
- اگه هم آیتم جایگزین از همون محصول (`resend_opts`) هم محصول دیگهٔ فعال با موجودی (`exch_opts`، فقط وقتی سفارش `active`ه) در دسترس باشن → یه دراپ‌داون «نوع عملیات» نشون داده می‌شه که با JS بین دو زیرفرم (همون فیلدهای قبلی resend/exchange، بدون تغییر در POST handlerها) سوییچ می‌کنه.
- اگه فقط یکی از این دو در دسترس باشه (رایج‌ترین حالت — مثلاً سفارش برگشتی که فقط ارسال‌جایگزین معنی داره، نه تعویض) → مستقیم همون یه فرم نشون داده می‌شه، بدون دراپ‌داون اضافهٔ بی‌مورد.
- اگه هیچ‌کدوم در دسترس نباشه → پیام «در حال حاضر نه آیتم جایگزین از همین محصول و نه محصول دیگری با موجودی در دسترس است».
این منطق دقیقاً همون هدف «هر قابلیت تکراری/غیرضروری رو حذف کن» رو با کمترین کلیک برای رایج‌ترین حالت‌ها پیاده می‌کنه، بدون این‌که هیچ سناریوی قبلاً پشتیبانی‌شده از دست بره.

**تب برگشت** فقط وقتی سفارش `active`ه فعاله؛ برای سفارش برگشتی به‌جاش پیام راهنما («این سفارش قبلاً برگشت خورده — برای ارسال کالای جدید از تب «تعویض / ارسال جایگزین» استفاده کنید») نشون داده می‌شه و تب پیش‌فرض روی «تعویض/جایگزین» باز می‌شه.

**لیست سفارش‌ها (`orders_list`):** تابع محلی `payment_note()` (بخش ۴۱) کاملاً حذف شد — ستون وضعیت فقط `order_status_badge()` خام رو نشون می‌ده. بخش لاگ فعالیت پایین صفحه از یه `<div>` ساده به `<details open>` (کشویی، قابل جمع‌شدن با کلیک روی `<summary>`) تبدیل شد؛ داخلش یه `<div class="max-h-[300px] overflow-y-auto">` — تقریباً ۵ ردیف بدون اسکرول قابل‌دیدن، بقیه با اسکرول (تعداد واکشی‌شده هم از ۱۵ به ۵۰ افزایش یافت، چون دیگه صفحه رو شلوغ نمی‌کنه).

**رندر خواناتر لاگ (`_order_log_row_html`):** به‌جای یه خط متن خام پایپ‌جدا، هر ردیف حالا: آیکون + برچسب اکشن + شمارهٔ سفارش (لینک مستقیم به هاب، برای تعویض هر دو شماره با فلک `→`) + تاریخ + نام ادمین در یه خط، و جزئیات (`details`) به چیپ‌های جدا (`key: value`) تبدیل می‌شه؛ یه حاشیهٔ رنگی سمت راست بر اساس نوع اکشن (قرمز=برگشت، کهربایی=تعویض، سبز=ارسال‌مجدد، آبی=ویرایش) برای اسکن بصری سریع‌تر. تابع تازهٔ `_order_log_value_label()` کدهای داخلی (`restore`, `custom_add`, `wrong_product`, ...) رو به برچسب فارسی خوانا ترجمه می‌کنه؛ مقادیر پیوندی مثل `custom_add(+1,500)` هم پوشش داده می‌شن (پیشوند شناسایی می‌شه، باقی رشته دست‌نخورده می‌مونه).

**`_fetch_order_logs`** ساده‌سازی شد — شاخهٔ per-order (`oid` filter) که فقط برای کارت تاریخچهٔ حذف‌شدهٔ هاب استفاده می‌شد، حذف شد؛ امضا از `_fetch_order_logs(conn, oid=None, limit=20)` به `_fetch_order_logs(conn, limit=50)` ساده شد.

⚠️ **نکتهٔ جانبی کشف‌شده حین تست (نه یک باگ تازه، رفتار از قبل موجود):** `resend_items` توی هاب با `WHERE p.title=order['title']` مچ می‌شه (نه `product_id`) — یعنی اگه عنوان سفارش از طریق کارت «ویرایش» به چیزی غیر از عنوان واقعی محصول تغییر کنه، «ارسال آیتم جایگزین» دیگه هیچ آیتمی پیدا نمی‌کنه (چون دیگه با هیچ محصولی مچ نمی‌شه). این محدودیت از قبل در کد وجود داشت (بخش ۴۲ هم دست‌نخورده گذاشتش) و خارج از اسکوپ این دور اصلاح بود — فقط مستندسازی شد.

**تست انجام‌شده:** هارنس مستقیم — ده-مرحله‌ای کامل: ویرایش، رندر هاب فعال (تأیید نبود کارت اطلاعات/تاریخچه)، برگشت با کسر دلخواه، رندر هاب برگشتی (تأیید پیام تب برگشت + فرم مستقیم ارسال‌جایگزین بدون دراپ‌داون اضافه چون تعویض در دسترس نیست)، ارسال مجدد با افزایش دلخواه، رندر هاب فعال دوباره، سناریوی هر دو گزینه در دسترس (تأیید ظهور دراپ‌داون «نوع عملیات»)، تعویض کامل، رندر هاب سفارش جدید، و رندر لیست سفارش‌ها (تأیید نبود بج‌های `payment_note` قدیمی در ستون وضعیت + وجود `<details>`/کانتینر اسکرول‌دار + وجود ردیف‌های لاگ). `python3.12 -m py_compile admin_panel.py` بدون خطا.

فایل‌های تغییرکرده: `admin_panel.py`.

---

## ۴۴. حذف کامل کارت «ویرایش اطلاعات پایه» از «وضعیت سفارش» (۲۰۲۶-۰۸-۰۷، ادامهٔ همون روز)

مالک پروژه بعد از بازطراحی دوم (بخش ۴۳) خواست کارت «ویرایش اطلاعات پایه» (عنوان/قیمت سفارش) هم از هاب «وضعیت سفارش» کاملاً حذف بشه — چون اطلاعات پایهٔ محصول جای درستش بخش محصول/موجودیه، نه اینجا؛ صریحاً تأکید کرد بقیهٔ صفحه دست‌نخورده بمونه.

رفع: `edit_card` و ارجاعش در `body` از `order_status_page` حذف شدن. چون فرم این کارت تنها راه رسیدن به `POST /orders/{oid}/edit` بود و هیچ لینک/فرم دیگری در کل پروژه بهش وصل نمی‌شد (چک شد با grep)، این روت هم به‌عنوان کد مرده حذف شد. `db.order_update()` (تک caller‌ش همین روت بود) عمداً دست‌نخورده موند — طبق تأکید صریح «بقیش رو دستکاری نکن»، حذفش یه قدم فراتر از دقیقاً همون چیزی بود که خواسته شده بود، پس محافظه‌کارانه نگه داشته شد (بی‌خطر، فقط بدون caller).

**تست انجام‌شده:** هارنس مستقیم — تأیید حذف کامل `order_edit_post` از ماژول، رندر هاب بدون کارت ویرایش (و بدون برگشت اطلاعات سفارش/تاریخچه که در بخش ۴۳ حذف شده بودن)، و سالم‌ماندن کامل جریان برگشت + رندر لیست سفارش‌ها. `python3.12 -m py_compile admin_panel.py` بدون خطا.

فایل‌های تغییرکرده: `admin_panel.py`.

---

## ۴۵. ممیزی سریع کل پروژه + رفع ۶ مورد امنیتی و ۵ مورد کارایی (۲۰۲۶-۰۸-۰۷)

طبق درخواست صریح مالک پروژه («کل پروژه رو سریع بررسی کن، از فایل md استفاده کن»)، به‌جای خوندن کامل کد، ممیزی مبتنی بر بخش‌های ۱۳/۱۴ همین سند + وریفای مستقیم چند نقطهٔ کلیدی انجام شد. بعد با تأیید «تمام موارد رو با درایت خودت اصلاح کن»، همهٔ یافته‌های قابل‌رفع بدون ریسک لاک‌اوت/شکستن یکپارچگی رفع شدن. جزئیات کامل هر مورد در `CHANGELOG_AI.md` (entry «بررسی کامل پروژه + رفع ۶ مورد امنیتی و ۵ مورد کارایی») ثبت شده؛ خلاصهٔ چیزی که برای کار روی این بخش‌ها لازمه بدونی:

### امنیتی — همهٔ موارد باز بخش ۱۳ (به‌جز ۱۰) رفع شدن
بند‌های ۲ تا ۶ و ۸ بخش ۱۳ همین سند حالا ✅ رفع‌شده‌ان (متن کامل هرکدوم بالاتر در همون بخش). نکات معماری کلیدی که برای کار آینده روی auth پنل لازمه بدونی:

- **`_hash_pw`/`_verify_pw`/`_verify_super_pw`** (admin_panel.py، بالای فایل، کنار `_pbkdf2_hash`) — تک‌منبع حقیقت تازهٔ هش پسورد. هر جای دیگه‌ای که پسورد ادمین چک/ذخیره می‌شه باید از این سه تابع استفاده کنه، نه SHA256 دستی.
- **الگوی ارتقای خودکار هش (auto-upgrade-on-login)** — این پروژه جای دیگه‌ای هم این الگو رو داره (مثلاً section ۲۸ فاز R برای rate-limit). هر migration فرمت داده‌ای که نیاز به «پسورد خام لحظه‌ای» داره (نه چیزی که بعداً batch قابل انجام باشه) باید همین الگو رو تکرار کنه: چک فرمت قدیمی → وریفای با فرمت قدیمی → اگه موفق، بازنویسی با فرمت جدید همون لحظه.
- **بای‌پس `admin`/`super` مشروط شد، نه حذف کامل** — تصمیم عمدی برای جلوگیری از لاک‌اوت کسی که هیچ‌وقت `ADMIN_WEB_USERNAME` رو صریحاً تنظیم نکرده. اگه در آینده خواستی این بای‌پس رو کاملاً حذف کنی، باید از قبل مطمئن بشی مالک پروژه یه یوزرنیم سفارشی واقعی تنظیم کرده.
- **`_csrf_guard`** (payment_service.py، کنار `_security_headers`) — Origin/Referer validation، نه token واقعی. اگه در آینده تصمیم به CSRF token واقعی per-form گرفته شد، این میدل‌ور می‌تونه به‌عنوان لایهٔ دوم باقی بمونه (defense-in-depth)، نیازی به حذفش نیست.
- **`X-Internal-Secret` روی مسیر `API_KEYS` در `api.py`** — همون `config.INTERNAL_API_SECRET` که `/payment/create` رو محافظت می‌کنه (بند ۱۱)؛ اگه در آینده یه کلاینت داخلی واقعی این مسیر auth رو استفاده کرد، باید این هدر رو هم بفرسته (دقیقاً همون الگوی `services/payments.py`).

### کارایی — ایندکس‌های گمشدهٔ بخش‌های ۳۶/۴۰/۴۳ + DDL تکراری
- `idx_feed_order_id` (`product_feed(order_id)`) و `idx_tickets_order_id` (`tickets(order_id)`) به `db.py` اضافه شدن — لوکاپ معکوس برگشت/تعویض (بخش ۴۰) و بستن تیکت وابسته (بخش ۳۶) قبلاً بدون ایندکس full-scan می‌کردن.
- `idx_admin_logs_section` روی `admin_logs(section, id)` — کوئری لاگ فعالیت لیست سفارش‌ها (بخش ۴۳).
- `_log()`/`_fetch_order_logs()` قبلاً هر فراخوانی یک DDL (CREATE TABLE/ALTER/INDEX) اجرا می‌کردن؛ حالا با فلگ مشترک per-process `_ADMIN_LOGS_SCHEMA_READY` (تعریف‌شده کنار `_log()`، بالای فایل) فقط یک‌بار در عمر پروسه امتحان می‌شن — دقیقاً الگوی `_INDEXES_DONE` در `db.py` (بخش ۲۴).

⚠️ **عمداً رفع نشد:** پرونینگ خودکار `admin_logs` (رشد نامحدود، هر اکشن ادمین یک ردیف) — چون تنها مکانیزم پاکسازی خودکار موجود (`/admin/system-cache`، بخش ۳۷) صریحاً از دست‌زدن به دیتابیس منع شده؛ ساخت یه مکانیزم حذف خودکار جداگانه بدون تأیید صریح روی یه جدول ممیزی، ریسک از‌دست‌رفتن داده بدون درخواست مشخص بود.

### موارد بررسی‌شده که همچنان باز و مستندن (بدون تغییر در این دور)
`db.add_product()` باگ `cols`، `products.chat_enabled` هیچ‌وقت ساخته نمی‌شه، کد مردهٔ `_process_discount_code`/`apply_discount_*`، ۵۱ فیلد `type="number"` بدون پشتیبانی کیبورد فارسی — همه در بخش ۱۴ همین سند از قبل مستندن و در این دور دست‌نخورده موندن (خارج از اسکوپ ممیزی سریع/ریسک بالاتر برای رفع یکجا).

**تست انجام‌شده:** هارنس مستقیم برای هر ۸ مورد رفع‌شده — لاگین با/بدون یوزرنیم سفارشی (۴ سناریو)، ارتقای خودکار هش قدیمی→PBKDF2 + لاگین دوباره بعد از ارتقا، نوشتن هش (نه plaintext) در `.env` هنگام تغییر رمز سوپرادمین، ساخت موفق هر دو ایندکس تازهٔ `db.py`، اجرای بدون خطای `_fetch_order_logs`/`_log` + ساخت `idx_admin_logs_section`، میدل‌ور CSRF (۶ سناریو)، `_auth` در `api.py` (۴ سناریو). سینتکس کامل هر ۴ فایل + اسموک‌تست ایمپورت.

فایل‌های تغییرکرده: `admin_panel.py`، `db.py`، `payment_service.py`، `api.py`.

---

## ۴۶. تکمیل واقعی مهاجرت Postgres — رفع کد اپلیکیشن که هنوز مستقل از DB_DIALECT بود (۲۰۲۶-۰۸-۰۷)

مالک پروژه اعلام کرد از مدت‌ها قبل به Postgres مهاجرت کامل کرده (`DB_DIALECT=postgres` + `DATABASE_URL` در `.env` سرور) و خواست هر ردپای SQLite قدیمی/منسوخ پیدا و پاک بشه. بررسی مستقیم کد قبل از هر اقدام نشون داد این یه درخواست «پاک‌سازی کد مرده» ساده نبود — **بخش بزرگی از اپلیکیشن اصلاً از لایهٔ Postgres عبور نمی‌کرد** و مستقل از `DB_DIALECT` مستقیم `sqlite3.connect()` می‌زد؛ یعنی روی سرور واقعی، این بخش‌ها روی یک فایل SQLite کاملاً فانتوم/جدا از دادهٔ واقعی Postgres کار می‌کردن. `db.py` (تک‌منبع مرکزی) از قبل درست از طریق `db_conn.get_connection()` مسیریابی می‌شد؛ مشکل جاهای دیگه بود.

### فاز ۱ — مسیریابی اتصال (۳۷ نقطهٔ `sqlite3.connect()` خام)
- **`admin_panel.py:_db()`** — مرکزی‌ترین تابع فایل (صدها بار در کل ~۱۱هزار خط صدا زده می‌شه) — یعنی تقریباً کل پنل ادمین روی دادهٔ فانتوم کار می‌کرد.
- **`bot.py`** — ۱۶+ نقطهٔ پراکنده (برخی با import مستقیم `sqlite3`، برخی با alias مثل `_sq`/`_sq4`/`_sqe`/`_sqr` — که در بازرسی اولیهٔ سطحی به‌راحتی از قلم می‌افتن). تابع مشترک `_db_conn()` که از قبل وجود داشت هم خودش راه SQLite خام می‌رفت. همهٔ این نقاط به `_db_conn()` یکپارچه شدن و خودِ `_db_conn()` به `db_conn.get_connection()` وصل شد. سه مورد import بلااستفادهٔ `sqlite3`/`DB_PATH` (بدون هیچ استفادهٔ واقعی در بدنهٔ تابع) هم به‌عنوان کد مرده حذف شدن.
- **`payment_service.py:db_connect()`** — مسیر تأیید پرداخت/تحویل محصول.
- **`state.py`** — `ensure_admin()`/`admin_has_perm()`: یعنی ادمین‌های ثانویه (غیر از `ADMIN_ID` سوپرادمین) روی Postgres همیشه رد دسترسی می‌گرفتن.
- **`db.py`**: تابع `count_user_product_orders_today` **دوبار تعریف شده بود** (نسخهٔ اول کاملاً مرده/سایه‌خورده، حذف شد)؛ نسخهٔ زنده هم SQLite خام + `sqlite_master`/`PRAGMA table_info` بدون مسیریابی بود — بازنویسی کامل با `_get_connection()`.

### فاز ۲ — باگ‌هایی که فقط با **تست مستقیم روی Postgres واقعی** کشف شدن (نه با خوندن کد)
چون هیچ Postgres واقعی‌ای در دسترس قبلی سشن‌ها نبوده، همهٔ سشن‌های قبلی (از جمله همین سشن، قبل از این نقطه) فرض SQLite رو تست می‌کردن. برای این دور، یک Postgres 16 واقعی (نصب مستقیم `apt install postgresql`، نه mock) + `psycopg2-binary` نصب شد و `init_db()`/توابع کلیدی واقعاً روش اجرا شدن. سه باگ **کاملاً جدید و قبلاً مستندنشده** پیدا شد:

1. **Transaction poisoning روی هر `ALTER TABLE ADD COLUMN` تکراری.** الگوی استاندارد پروژه (بخش ۷ سند: `ALTER TABLE ... ADD COLUMN` در `try/except: pass`) روی SQLite بی‌خطره چون هر دستور شکست‌خورده مستقل از بقیه است. روی Postgres، **یک دستور شکست‌خورده کل تراکنش رو «poison» می‌کنه** — هر دستور بعدی روی همون کانکشن با `InFailedSqlTransaction` شکست می‌خوره، حتی دستورهای کاملاً بی‌ربط. یعنی `init_db()` روی هر ری‌استارت *بعد از اولین بار* (وقتی ستون‌ها از قبل وجود دارن) کامل کرش می‌کرد. رفع: `db_dialect.translate()` حالا `ALTER TABLE ... ADD COLUMN` رو به `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` تبدیل می‌کنه (فقط زیر Postgres؛ SQLite این سینتکس رو نداره، دست‌نخورده موند) — سینتکس idempotent بومی پستگرس، اصلاً خطا تولید نمی‌کنه، نیازی به تکیه به catch کردن نیست.
2. **`RealDictCursor` فقط `row['col']` رو پشتیبانی می‌کرد، نه `row[0]`.** کد موجود پروژه (db.py و همه‌جای دیگه) جاهای زیادی با اندیس عددی (`row[0]`) به نتیجهٔ کوئری دسترسی داره — دقیقاً مثل `sqlite3.Row` که هر دو حالت رو پشتیبانی می‌کنه. `db_conn.py` از `RealDictCursor` استفاده می‌کرد (فقط دیکشنری) که با هر `row[0]` یه `KeyError` می‌داد. رفع: جایگزین با `psycopg2.extras.DictCursor` که (تأیید با تست مستقیم) دقیقاً همون رفتار دوگانهٔ `sqlite3.Row` رو داره.
3. **`cursor.lastrowid` همیشه `None` بود.** SQLite خودکار `lastrowid` رو بعد از هر INSERT می‌ده؛ Postgres نیاز به `RETURNING` صریح توی خودِ INSERT داره — چیزی که در **~۳۶ نقطه** (`db.py`، `iphone_valuation/db.py`، `payment_service.py`) هیچ‌کدوم نداشتن (مثلاً `add_product()` که همین امروز در بخش «اصلاحیه» این سند رفع شده بود، بدون این رفع دوم هنوز `None` برمی‌گردوند). رفع ریشه‌ای بدون دست‌زدن به ۳۶ نقطه: `_PgCursor.lastrowid` حالا `SELECT lastval();` رو صدا می‌زنه — معادل دقیق SQLite `lastrowid` (مقدار سکانسی که آخرین `nextval()` در همین سشن گرفته، که هر INSERT روی ستون SERIAL خودکار صداش می‌زنه) — بدون نیاز به تغییر خودِ ۳۶ INSERT.

### فاز ۳ — استثناهای خاص SQLite که زیر Postgres هیچ‌وقت catch نمی‌شدن
- `except sqlite3.OperationalError` (۱۴ مورد، همه محافظ `ALTER TABLE ADD COLUMN`) — با رفع فاز ۲.۱ (`IF NOT EXISTS`) دیگه هیچ‌وقت لازم نیست trigger بشن؛ خودِ except دست‌نخورده موند.
- `except sqlite3.IntegrityError` (۴ مورد در `db.py`/`admin_panel.py`، برای مدیریت نقض یکتایی مثل authority تکراری تراکنش/یوزرنیم تکراری ادمین) — psycopg2 کلاس استثنای کاملاً جدایی داره (`psycopg2.IntegrityError`، هر دو طبق استاندارد DB-API 2.0). تاپل سازگار `_INTEGRITY_ERRORS = (sqlite3.IntegrityError, psycopg2.IntegrityError)` (با fallback امن اگه psycopg2 نصب نبود) در `db.py`/`admin_panel.py` اضافه شد.

### فاز ۴ — `INSERT OR REPLACE` → `ON CONFLICT` (به‌جای وصله‌کاری db_dialect)
`db_dialect.py` از قبل `INSERT OR REPLACE` رو به یه `INSERT` سادهٔ بی‌اثر (نه واقعاً REPLACE) ترجمه می‌کرد — باگ مستندشدهٔ قدیمی سند (بخش ۱۴). به‌جای تلاش برای حدس زدن generic ستون تعارض با regex (ریسک‌دار برای کلید‌های ترکیبی)، **هر ۹ نقطهٔ واقعی استفاده** (`db.py`, `bot.py`, `payment_service.py`, `admin_panel.py`, `api.py` — جدول‌های `bot_config`, `admin_preferences`, `delivery_messages`, `pending_deliveries`, `partner_bank_info`) مستقیماً به سینتکس پرتابل `INSERT ... ON CONFLICT(col) DO UPDATE SET ...` بازنویسی شدن — این سینتکس **هم روی SQLite (≥3.24) هم Postgres** عیناً یکسان کار می‌کنه، دیگه نیازی به ترجمهٔ زمان‌اجرا نداره. `admin_preferences` کلید ترکیبی (`admin_id,key`) داشت — `ON CONFLICT(admin_id,key)` درست همینو پوشش می‌ده.

### فاز ۵ — `SELECT changes()` → `cursor.rowcount`
سه نقطه (`subscribe_stock`, `save_rating`, `save_product_rating`) از `SELECT changes()` (خاص SQLite، روی Postgres همیشه با ترجمهٔ قدیمی `SELECT 1` یعنی همیشه truthy) برای فهمیدن «آیا `INSERT OR IGNORE` واقعاً درج کرد یا نادیده گرفته شد» استفاده می‌کردن. رفع با `cursor.rowcount` (تست مستقیم: هم `sqlite3.Cursor` هم `_PgCursor` درست ۱/۰ برمی‌گردونن) — پرتابل، بدون نیاز به ترجمه.

### فاز ۶ — سیستم بکاپ (بزرگ‌ترین ریسک عملیاتی کشف‌شده)
`stbak_engine.py` (موتور بکاپ اصلی پنل) فقط روی SQLite کار می‌کنه — یعنی همهٔ مسیرهای `/admin/database/*` (بکاپ دستی، بکاپ خودکار روزانه، بازیابی، Recovery اضطراری، factory reset) قبل از این رفع داشتن **بی‌صدا روی فایل SQLite فانتوم بکاپ می‌گرفتن/بازیابی می‌کردن** — نه شکست آشکار، بلکه ظاهراً موفق ولی کاملاً بی‌ربط به دادهٔ واقعی Postgres. این از هر باگ دیگهٔ این دور خطرناک‌تر بود (توهم امنیت بکاپ، بدون بکاپ واقعی).

ماژول `pg_backup.py` (بر پایهٔ `pg_dump`/`psql`، مسیر Postgres) از قبل در پروژه وجود داشت ولی dormant/wired-نشده بود. رفع:
- `_do_auto_backup()` (بکاپ خودکار روزانه) حالا بر اساس `db_conn.is_postgres()` سوییچ می‌کنه — Postgres → `pg_backup.run_full_backup()`، SQLite → رفتار قبلی `stbak_engine` دست‌نخورده.
- ۸ روت POST مبتنی بر `stbak_engine` (`backup/full-sync`, `restore-auto`, `recover-latest`, `restore/sync`, `reset/sync`, `backup/start`, `restore/start`, `reset/start`) با تابع تازهٔ `_stbak_pg_guard()` محافظت شدن — زیر Postgres با پیام خطای واضح مسدود می‌شن (به‌جای «موفقیت» بی‌معنی روی فایل نامرتبط).
- دو روت تازه: `POST /database/pg-backup/run` (بکاپ کامل دستی Postgres) و `POST /database/pg-backup/restore` (بازیابی از بکاپ Postgres محلی).
- کارت تازهٔ «🐘 بکاپ/بازیابی Postgres» بالای صفحهٔ `/database` — فقط زیر Postgres نمایش داده می‌شه؛ لیست «بکاپ‌های خودکار» موجود هم دیالوگ‌آگاه شد (`pg_backup.list_local_backups()` زیر Postgres، `stbak_engine` زیر SQLite).
- روت دانلود بکاپ (`/database/download/{filename}`) هم هر دو مسیر (`_BACKUP_DIR` برای SQLite، `pg_backup.BACKUP_DIR` برای Postgres — مسیرهای دیسک متفاوت) رو چک می‌کنه.
- ANALYZE هفتگی (کرون قدیمی، مخصوص برنامه‌ریز کوئری SQLite) فقط زیر SQLite اجرا می‌شه — Postgres خودش autovacuum/analyze خودکار داره.

⚠️ **محدودیت شناخته‌شده:** `pg_backup.py` فقط بکاپ/بازیابی **کامل** (whole-database dump) پشتیبانی می‌کنه، نه بکاپ بخش‌بندی‌شده/انتخابی مثل `stbak_engine` (که ادمین می‌تونست فقط چند جدول خاص رو بکاپ/ریست کنه). اگه بازیابی بخش‌بندی‌شدهٔ Postgres لازم شد، نیاز به کار جداگانه‌ای داره (`pg_dump --table=...` per-module) — فعلاً عمداً خارج از اسکوپ این رفع (که هدفش تضمین وجود *یک* بکاپ واقعاً کارکننده بود، نه فیچرپریتی).

### وابستگی‌های تازه
- `requirements.txt`: `psycopg2-binary>=2.9,<3` اضافه شد — **قبلاً حتی وقتی `DB_DIALECT=postgres` تنظیم شده بود، این پکیج در requirements نبود** (یعنی `db_conn.py`'s `_PgConnection.__init__` با `ModuleNotFoundError` شکست می‌خورد مگر کسی دستی `pip install` کرده باشه، خارج از ردگیری).
- سرور نیاز به پکیج سیستمی `postgresql-client` داره (برای `pg_dump`/`psql`، بخش ۱۲ سند).

### روش تست — Postgres واقعی، نه mock
برخلاف بقیهٔ این سند (که همیشه هارنس مستقیم روی SQLite موقت بوده، چون هیچ Postgres واقعی در دسترس نبود)، این دور یک **Postgres 16 واقعی نصب شد** (`apt install postgresql` + `psycopg2-binary`، هر دو برای Python 3 و 3.12) و هر رفع مستقیماً روش تست شد: `init_db()` فرش + idempotent (ری‌استارت دوم)، `add_product`/`chat_enabled`/`subscribe_stock`/`save_rating`/`save_product_rating`/`set_maintenance_mode`/`count_user_product_orders_today`، `admin_panel._db()` (insert/select/verify_pw کامل)، `payment_service.db_connect()`، `state.ensure_admin`/`admin_has_perm`، `iphone_valuation.db.create_series` (lastrowid)، هر ۳ جدول با `ON CONFLICT` (کلید ساده + ترکیبی)، و کل مسیر بکاپ Postgres (`pg_backup_run`→`pg_backup_restore` روی روت‌های واقعی پنل، رندر صفحه با/بدون کارت Postgres، مسدودشدن روت‌های قدیمی). موازی، رگرسیون کامل SQLite هم بعد از هر تغییر دوباره اجرا شد (init_db، add_product، chat_enabled، subscribe_stock، maintenance_mode، `admin_panel._db()`) — بدون هیچ تغییر رفتار.

⚠️ **محدودیت این تست نسبت به سرور واقعی:** نسخهٔ Postgres تست‌شده (۱۶، نصب پیش‌فرض Ubuntu) و پیکربندی محلی (بدون SSL/replication/pooler مثل PgBouncer) ممکنه با سرور تولید واقعی فرق داشته باشه. قبل از تکیهٔ کامل، توصیه می‌شه یک بار روی یک کپی/staging از دیتابیس واقعی هم این مسیرها (خصوصاً بکاپ/بازیابی) دستی تأیید بشن.
...
## ۴۷. تکمیل مهاجرت Postgres — باگ‌های باقی‌ماندهٔ «کاربران» + «کارشناسی آیفون» (۲۰۲۶-۰۸-۰۷)

مالک پروژه بعد از بخش ۴۶ گزارش داد بخش «موجودی کاربران» و «کارشناسی آیفون» در پنل هنوز خطا می‌دن و خواست هر ردپای باقی‌ماندهٔ SQLite بررسی بشه. به‌جای حدس، یک هارنس تست ساخته شد: `starlette.testclient.TestClient` روی `payment_service.app` + یک **Postgres 16 واقعی** نصب‌شده در سندباکس + دادهٔ seed واقعی (کاربر/کیف‌پول/محصول/فید/سفارش/سری آیفون)، که هر ۶۵ مسیر GET استاتیک پنل رو با لاگین واقعی می‌گشت. ⚠️ **نکتهٔ فنی تست:** `TestClient` باید با `base_url="https://testserver"` ساخته بشه نه `http://` — چون کوکی سشن ادمین `Secure` است (بخش ۱۳ آیتم ۱۲) و RFC 6265 روی `http` منتقل نمی‌شه، وگرنه لاگین موفق ولی هر درخواست بعدی انگار لاگین نیست. نکتهٔ دوم: مسیر `/logout` اگه توی لیست مسیرهای تست شده (حتی به‌ترتیب الفبایی) قرار بگیره، سشن رو نابود می‌کنه و هر مسیر بعدش کاذب «۳۰۳ (ریدایرکت لاگین)» نشون می‌ده — نه یه خطای واقعی؛ باید از لیست حذف بشه یا برای هر مسیر session جدا لاگین بشه.

این هارنس + بررسی مستقیم کد، این باگ‌ها رو پیدا کرد (همه رفع شدن):

1. **`feed_batches` (جدول لیزی) بدون `ensure_feed_batch_schema()` قبل از کوئری** در `get_accounting_kpis`/`get_product_accounting` (db.py) — رفع با صدا زدن اون تابع اول + `conn.rollback()` تدافعی در هر `except` (transaction poisoning، همون کلاس باگ بخش ۴۶).
2. **صفحهٔ «کاربران» (`/admin/users`)** — `orders.user_id` (INTEGER) با `CAST(u.user_id AS TEXT)` جوین می‌شد (اشتباه محض، هر دو ستون از قبل INTEGERان؛ روی SQLite با قوانین affinity بی‌اثر بود، روی Postgres `operator does not exist`) + `GROUP BY u.user_id` با `SELECT ... w.balance` بدون aggregate (Postgres GROUP BY strictness، بخش ۷ سند اصلی). رفع: حذف CAST غیرلازم + `COALESCE(MAX(w.balance),0)`.
3. **`/admin/feed`** — `GROUP BY p.id` با `SELECT p.title, p.category, fas.threshold` → `GROUP BY p.id, p.title, p.category, fas.threshold`.
4. **`/admin/iphone`** — `iphone_valuation/db.py:get_stats()`: `GROUP BY v.model_id` با `SELECT m.name` → `GROUP BY v.model_id, m.name`.
5. 🔴 **باگ بزرگ — موتور قیمت‌گذاری آیفون کاملاً از کار افتاده بود روی Postgres.** `resolve_capacity`/`get_capacity_exact` (`iphone_valuation/db.py`) برای مقایسهٔ NULL-safe چندبعدی (پارت/رنگ/سری‌اصالت/ظرفیت، بخش ۲۲.۷/۲۲.۸) از الگوی `col IS ?` استفاده می‌کردن. روی SQLite، `IS` یک NULL-safe equality *عمومیه* (با هر مقدار کار می‌کنه)؛ روی Postgres، `IS` فقط `NULL`/`TRUE`/`FALSE` قبول می‌کنه — `storage_id IS 162` مستقیماً `SyntaxError` می‌داد. یعنی **هر ثبت/خواندن قیمت (`upsert_capacity`, `resolve_capacity`) و در نتیجه کل `service.valuate()`** (پایهٔ ویزارد ربات + API کارشناسی) روی Postgres شکست می‌خورد — دقیقاً همون چیزی که مالک پروژه گزارش داد. رفع **در سطح ترجمهٔ دیالوگ**، نه فقط این دو تابع: `db_dialect.py` حالا هر الگوی `col IS ?` رو (قبل از تبدیل `?`→`%s`) به `col IS NOT DISTINCT FROM ?` ترجمه می‌کنه (معادل استاندارد ANSI SQL، NULL-safe روی هر دو دیالوگ) — هر الگوی مشابه آینده هم خودکار پوشش داده می‌شه.
6. **روت‌های Excel/صفحه غیرقابل‌دسترس — ترتیب ثبت route (بدون ربط به دیالوگ دیتابیس، preexisting):** `GET /admin/users/export.xlsx` و `GET /admin/products/duplicates` همیشه `422` می‌دادن چون FastAPI روت‌ها رو به ترتیب ثبت match می‌کنه و `/users/{uid}` (`uid: int`)/`/products/{pid}` قبل از این دو مسیر خاص در فایل تعریف شده بودن — `"export.xlsx"`/`"duplicates"` به‌عنوان مقدار `int` رد می‌شدن. یعنی **دکمهٔ خروجی اکسل کاربران و صفحهٔ محصولات تکراری هیچ‌وقت کار نمی‌کردن**، نه در Postgres نه در SQLite. رفع: هر دو تعریف route به قبل از معادل پارامتری‌شونشون منتقل شدن (بدون تغییر منطق داخلی). **درس برای آینده: هر route جدید با مسیر ثابت زیر یه prefix که یه route پارامتری (`{id}`) هم داره، باید قبل از اون پارامتری ثبت بشه، وگرنه بی‌صدا غیرقابل‌دسترس می‌مونه.**
7. **رفع دفاعی `/admin/tickets` روی نصب کاملاً تازه:** `ensure_ticket_archive_schema()` بدون تضمین وجود جدول `tickets` (که فقط `ticket_ensure_schema()` از `bot.py` می‌سازه) مستقیم `ALTER TABLE` می‌زد. رفع: صدا زدن `ticket_ensure_schema()` (idempotent) اول تابع.

**تست انجام‌شده:** کرالر روی هر ۶۵ مسیر GET استاتیک پنل (صفر خطا/exception بعد از رفع)؛ `iphone_valuation.service.valuate()` مستقیم با مدل/ظرفیت واقعی روی Postgres (قبل: کرش؛ بعد: موفق + idempotency `upsert_capacity` تأیید‌شده)؛ رگرسیون کامل SQLite موازی هر مرحله (بدون تغییر رفتار). دو مسیر دینامیک هم مستقیم چک شدن (`/users/{uid}`، `/orders/{oid}`، `/products/{pid}`، `/iphone/prices?edit=`، `/wallets/{uid}/history`) — همه سالم.

فایل‌های تغییرکرده: `db.py`، `admin_panel.py`، `db_dialect.py`، `iphone_valuation/db.py`.

---

## ۴۸. بازطراحی کامل صفحهٔ «پشتیبان‌گیری و بازیابی» — یوزرفرندلی + انتخاب سفارشی کامل زیر Postgres (۲۰۲۶-۰۸-۰۷)

مالک پروژه خواست `/admin/database` بازطراحی بشه: توضیحات اضافه/تکراری حذف بشه، بخش‌های تکراری (بکاپ خودکار، بازیابی دوگانه، بازگردانی اضطراری) ادغام/حذف بشن، و مهم‌تر — «انتخاب سفارشی» بکاپ که قبلاً فقط زیر SQLite کار می‌کرد، واقعاً زیر Postgres هم کار کنه.

### ساختار جدید صفحه (به‌جای ۵ کارت قدیمی: کارت Postgres جدا + بکاپ + بکاپ‌های خودکار + بازیابی پشتیبان + بازگردانی اضطراری + ریست)
1. **بکاپ** — یک کارت واحد (بدون پاراگراف توضیحی، فقط عنوان)، «انتخاب سفارشی» (تک‌تک ماژول‌ها) یا کامل، دیالوگ‌آگاه در پس‌زمینه.
2. **بازیابی** — یک کارت واحد: `<details>` کشویی «۵ بکاپ آخر» (هر ردیف: تاریخ+حجم+دکمهٔ دانلود+دکمهٔ بازیابی مستقیم) + زیرش گزینهٔ بارگذاری فایل بکاپ دستی از روی سیستم. هر دو مسیر (لیست/آپلود دستی) دیالوگ‌آگاهن.
3. **ریست سیستم** — دست‌نخورده در UI، ولی دیگه زیر Postgres بلاک نمی‌شه (پایین‌تر ببین).

**حذف کامل:** کارت جدای «🐘 بکاپ/بازیابی Postgres (pg_dump)» (با پاراگراف «این پروژه به Postgres مهاجرت کرده...»)، کارت «🕐 بکاپ‌های خودکار» (جدول جدا)، و کارت «🆘 بازگردانی اضطراری» — طبق درخواست صریح، این قابلیت‌ها یا داخل دو کارت بالا ادغام شدن یا (بازگردانی اضطراری) کلاً حذف شدن. روت بک‌اند `/database/recover-latest` دست‌نخورده موند (فقط از UI حذف شد، در صورت نیاز آینده هنوز در دسترسه).

**رفع تکرار نام‌گذاری/چک‌باکس:** چک‌باکس تکراری «💰 داده‌های حسابداری» (که برای بکاپ اصلاً هیچ اثری نداشت — فقط برای ریست معنی داشت، بررسی کد نشون داد `value="__accounting__"` هیچ‌جای مسیر بکاپ خونده نمی‌شد) از فهرست ماژول‌های **بکاپ** حذف شد (تابع تازهٔ `_mod_checks()`، بدون این چک‌باکس تکراری)؛ در **ریست** دست‌نخورده موند (`_chk()` قدیمی، چون واقعاً استفاده می‌شه و تغییرش خارج از درخواست بود).

### تکمیل «انتخاب سفارشی» بکاپ زیر Postgres (قبلاً محدودیت مستندشده در بخش ۴۶)
- `pg_backup.create_backup(tables=None)`/`run_full_backup(tables=None)` — پارامتر تازه؛ وقتی داده بشه، هر جدول با `-t table` به `pg_dump` اضافه می‌شه (بکاپ فقط همون جدول‌ها، نه کل دیتابیس).
- `POST /database/backup/start` حالا دیالوگ‌آگاهه: زیر Postgres، کلیدهای ماژول انتخاب‌شده (`sections`) با `stbak_engine.MODULES` به لیست جدول واقعی ترجمه می‌شن و به `pg_backup.create_backup(tables=...)` پاس داده می‌شن؛ زیر SQLite رفتار قبلی (`create_stbak`) دست‌نخورده. **مهم:** این مسیر عمداً از `run_full_backup()` (که به مقاصد ابری هم آپلود می‌کنه) استفاده نمی‌کنه — مستقیم `create_backup()` صدا می‌زنه، چون بکاپ دستی/سفارشی نباید خودکار به گوگل‌درایو/کانال تلگرام هم بره (همون رفتار قبلی SQLite که فقط دانلود می‌کرد، بدون آپلود ابری).
- `GET /database/backup/download/{job_id}` هم دیالوگ‌آگاه شد — زیر Postgres، فایلی که `pg_backup.create_backup()` از قبل توی `BACKUP_DIR` خودش ساخته و rotate کرده رو مستقیم serve می‌کنه (بدون ذخیرهٔ تکراری با نام‌گذاری SQLite در مسیر اشتباه، که قبلاً یه باگ بالقوه بود اگه این مسیر برای Postgres هم صدا زده می‌شد).

### تکمیل بازیابی زیر Postgres
- `POST /database/pg-backup/restore` (از قبل موجود) — بازیابی از یکی از ۵ بکاپ آخر لیست‌شده، بدون تغییر.
- `POST /database/pg-backup/restore-upload` (تازه) — معادل Postgres همون `/database/restore/start` (SQLite) برای «بارگذاری فایل بکاپ دستی از روی سیستم»؛ فایل آپلودی به یه مسیر موقت نوشته می‌شه، `pg_backup.restore_backup()` صداش می‌زنه، بعد پاک می‌شه.
- سمت کلاینت: دو متغیر JS تازه `RESTORE_URL`/`RESTORE_UPLOAD_URL` (سرور موقع رندر صفحه، بر اساس دیالوگ، مقداردهی می‌کنه) — یک تابع مشترک `doRestoreFile(filename)` (ادغام دو تابع قبلی `runAutoRestore`/`pgBackupRestore`) و یک `runRestore()` دیالوگ‌آگاه برای آپلود دستی، به‌جای ۴ تابع/۲ کارت جدا.

### تکمیل ریست زیر Postgres
`factory_reset` قدیمی (`stbak_engine.py`) کاملاً SQLite-محوره (`sqlite3.connect` خام، `PRAGMA foreign_keys`، `sqlite_sequence`). تابع تازهٔ `admin_panel._generic_factory_reset(modules)` معادل دیالوگ‌آگاهش رو با `db_conn.get_connection()` پیاده می‌کنه (فقط `DELETE FROM` روی جدول‌های ماژول‌های انتخاب‌شده، بدون هیچ‌کدوم از بخش‌های خاص SQLite که برای درستی ریست ضروری نیستن). `POST /database/reset/sync` دیگه `_stbak_pg_guard()` نداره — دیالوگ‌آگاهه (Postgres → `_generic_factory_reset`، SQLite → `factory_reset` قدیمی، رفتار قبلی دست‌نخورده).

⚠️ **`POST /database/reset/start`** (نسخهٔ job-based ریست) عمداً دست‌نخورده موند — بررسی شد که از هیچ‌جای صفحهٔ فعلی صدا زده نمی‌شه (کد مرده/orphaned از قبل، `runJob('reset')` فقط `/reset/sync` رو صدا می‌زنه)، پس ریسک اضافه نکردن این مسیر منطقی نبود.

**تست انجام‌شده:** هارنس مستقیم `TestClient` روی Postgres 16 واقعی + رگرسیون کامل SQLite — رندر صفحه (تأیید حذف کامل متن/کارت‌های قدیمی + وجود ساختار تازه)، بکاپ کامل end-to-end (job→دانلود)، بکاپ سفارشی (فقط ماژول «کاربران» — تأیید حجم فایل واقعاً کوچیک‌تر از بکاپ کامله، یعنی `-t` واقعاً محدود می‌کنه)، بازیابی از لیست ۵تایی + بازیابی از آپلود دستی (هر دو با تأیید سلامت داده بعدش)، ریست سفارشی (فقط یک ماژول بی‌ربط — تأیید عدم اثر روی جدول‌های دیگه). همه زیر هر دو دیالوگ سبز.

فایل‌های تغییرکرده: `admin_panel.py`، `pg_backup.py`.

---

## ۴۹. رفع بازیابی بکاپ‌های قدیمی ZIP-محور (.stbak) روی Postgres — تشخیص خودکار فرمت + خودترمیمی نوع ستون (۲۰۲۶-۰۸-۰۷)

مالک پروژه بعد از یه «ریست» سیستم، یه بکاپ قدیمی (فرمت اختصاصی ZIP-محور خودِ `stbak_engine.py`، نه فرمت جدید `pg_backup.py` که gzip از `pg_dump` است) رو خواست روی دیتابیس Postgres (که دیگه خالیه) بازیابی کنه — و «الان دیگه اینو نمی‌خونه». علت دقیق: `stbak_engine.restore_stbak()` (تنها راه خوندن این فرمت ZIP) همیشه `sqlite3.connect(db_path)` خام می‌زد — یعنی این بکاپ‌ها، حتی از طریق مستقیم کد، هیچ‌وقت روی Postgres قابل بازیابی نبودن، نه فقط از پنل.

### ۱) `restore_stbak()` دیالوگ‌آگاه شد
از `db_conn.get_connection()` استفاده می‌کنه (نه `sqlite3.connect` خام)؛ بکاپ ایمنی قبل از بازنویسی هم زیر Postgres با `pg_backup.create_backup()` گرفته می‌شه (قبلاً فقط SQLite). کامیت حالا per-table انجام می‌شه (نه یک تراکنش بزرگ نهایی) — چون روی Postgres، یک جدول ناموفق کل تراکنش رو poison می‌کنه (همون کلاس باگ بخش ۴۶)؛ `conn.rollback()` بعد از هر خطای جدولی، دقیقاً همون الگو.

### ۲) `/admin/database/pg-backup/restore-upload` حالا فرمت رو خودش تشخیص می‌ده
دو بایت اول فایل چک می‌شه (`PK` = ZIP → مسیر `stbak_engine.restore_stbak`؛ در غیر این صورت gzip → مسیر `pg_backup.restore_backup`). ادمین دیگه لازم نیست بدونه بکاپش کدوم فرمته — همون یک دکمهٔ «بازیابی از فایل» (بخش ۴۸) هر دو نوع رو قبول می‌کنه.

### ۳) `_run_all_schema_migrations()` تکمیل شد — از ۳ تابع به ~۳۵ تابع
نسخهٔ قبلی این تابع (که قبل/بعد از هر Restore صدا زده می‌شه تا ساختار دیتابیس به‌روز باشه) فقط `db.init_db`/`ensure_product_support_schema`/`iphone_valuation.ensure_schema` رو می‌شناخت. با تست بازیابی یه بکاپ واقعی مشخص شد این کاملاً ناکافیه — پروژه ~۳۰ تابع `ensure_*` جدا داره (بیشترشون در `db.py`، چندتاش هم فقط داخل خودِ یه route در `admin_panel.py` لیزی تعریف شده بودن، نه به‌عنوان تابع مستقل). روی یه Postgres که همهٔ این مسیرهای لیزی حداقل یک‌بار اجرا نشدن، بازیابی روی بیش از ۲۰ جدول با «relation does not exist» شکست می‌خورد. رفع:
- تقریباً هر `ensure_*` شناخته‌شدهٔ `db.py` به لیست اضافه شد.
- `payment_service.ensure_schema` هم اضافه شد.
- سه تابع تازه در `admin_panel.py` که قبلاً DDL‌شون فقط داخل یه route لیزی بود، حالا مستقل و قابل‌فراخوانی‌ان: `ensure_admin_preferences_schema()`، `ensure_wallet_admin_log_schema()`، `ensure_ticket_reply_columns()` — رفتار routeهای اصلی‌شون (`save_theme_pref`, `wallet_adjust`, `ticket_reply`) دست‌نخورده موند (کد تکراری عمدی، برای صفر ریسک روی routeهای حساس/تست‌شدهٔ موجود).
- `admin_panel._ensure_theme_table()` هم `ALTER TABLE admin_logs ADD COLUMN result` رو گرفت (قبلاً فقط داخل `_log()` بود، دقیقاً همون الگوی idempotent).

### ۴) 🔴 دو باگ نوع ستون که فقط با بازیابی دادهٔ واقعی کشف شدن
1. **`user_id`های تلگرام از سقف INTEGER چهاربایتی Postgres (۲.۱ میلیارد) بیشترن.** شناسه‌های تلگرام این روزها معمولاً بالای این عددن (مثال واقعی از همین بکاپ: `5686745580`). روی SQLite هیچ‌وقت مشکلی نبود (`INTEGER` در SQLite همیشه تا ۶۴بیت بدون شکایت جواب می‌ده)؛ روی Postgres با `integer out of range` شکست می‌خورد. ⚠️ این یعنی **هر جدولی که بعد از مهاجرت اولیه (`migrate_to_postgres.py`، که خودش `INTEGER`→`BIGINT` رو درست انجام می‌داد) از طریق مسیر زمان‌اجرا (`db_dialect.py`، که این عریض‌سازی رو نداره) ساخته شده باشه، همین الان هم روی سرور تولید همین مشکل رو داره** — نه فقط موقع Restore.
2. **`admin_logs.admin_id` اشتباهاً `INTEGER` تعریف شده بود، در حالی که همیشه TEXT است.** `_get_admin()` همیشه رشته برمی‌گردونه؛ برای سوپرادمین بوت‌استرپ این رشته literal `"super"` است (نه یه عدد). روی SQLite بی‌اثر بود؛ روی Postgres با `invalid input syntax for type integer: "super"` شکست می‌خورد — یعنی **لاگ فعالیت سوپرادمین بوت‌استرپ روی Postgres همیشه بی‌صدا گم می‌شد** (`_log()` هیچ‌وقت exception نمی‌ده). رفع ریشه‌ای: ستون به `TEXT` تغییر کرد (هم در `CREATE TABLE` هم `ALTER COLUMN` برای جدول‌های از‌قبل‌موجود).

**رفع سطح restore (خودترمیمی، دفاع در عمق برای هر مورد مشابه کشف‌نشده):** `stbak_engine._widen_integer_columns(conn, table, target_type)` — وقتی insert با «out of range» شکست بخوره، ستون(های) INTEGER همون جدول به `BIGINT` عریض می‌شن و دوباره امتحان می‌شه؛ وقتی با «invalid input syntax for type integer» شکست بخوره، به `TEXT` عریض می‌شن. بدون نیاز به دونستن از قبل کدوم جدول/ستون درگیره.

**تست انجام‌شده:** بازیابی مستقیم یه بکاپ واقعیِ ۸۹۳ رکوردی (ارسال‌شده توسط مالک پروژه) روی یه Postgres تازهٔ سندباکس — قبل از رفع: ۲۱ جدول خطا (۶۱۵-۷۲۰ از ۸۹۳ رکورد بازیابی‌شده، بسته به مرحلهٔ رفع). بعد از رفع کامل: **هر ۸۹۳ رکورد، صفر خطا**، شامل فایل‌های مدیا (۳۰ فایل). کرالر کامل ۶۵ مسیر پنل + رگرسیون round-trip کامل SQLite (بکاپ→بازیابی، صفر خطا) بعد از این تغییرات هم دوباره اجرا شد — بدون تغییر رفتار.

فایل‌های تغییرکرده: `stbak_engine.py`، `admin_panel.py`.

---

## ۵۰. رفع fallback خاموش به SQLite در `db.py._get_connection()` + ممیزی نهایی قبل از معرفی به مشتری واقعی (۲۰۲۶-۰۸-۰۷)

مالک پروژه صریح گفت: هدف فقط اینه که پروژه پایدار روی Postgres بمونه (نه شیفت به دیتابیس دیگه) و می‌خواد ربات رو به مشتری واقعی معرفی کنه بدون این‌که به مشکل بخوره. به‌جای حذف کد پشتیبانی SQLite (که فقط برای dev/تست محلی نگه داشته شده، بخش ۱ — حذفش یعنی از دست دادن توانایی تست بدون یه Postgres واقعی، بدون هیچ فایدهٔ امنیتی/کارایی برای تولید)، یک ممیزی هدفمند روی مسیرهای **مشتری‌محور** (`bot.py`, `api.py`, `core/`, `db.py`) انجام شد — جاهایی که کرالر قبلی (فقط مسیرهای GET پنل ادمین) هرگز پوششش نمی‌داد.

### 🔴 باگ پیدا‌شده — `db.py:_get_connection()` بی‌صدا به یه فایل SQLite فانتوم fallback می‌کرد
این تابع، پرکاربردترین نقطهٔ اتصال کل پروژه (صدها فراخوان در `db.py`)، یه بلوک `try/except Exception:` داشت که اگه `db_conn.get_connection()` به هر دلیلی (قطعی موقت شبکهٔ Postgres، DATABASE_URL موقتاً نامعتبر، هر باگ گذرا) استثنا می‌داد، **بی‌صدا** به یه اتصال SQLite محلی fallback می‌کرد. زیر Postgres، این یعنی به‌جای شکست آشکار (قابل لاگ/آلارم/ریترای)، اپ ساکت روی یه دیتابیس فانتومِ کاملاً جدا از دادهٔ واقعی ادامه می‌داد — دقیقاً همون کلاس باگی که بخش ۴۶ رو به‌وجود آورده بود (`pg_backup`, `admin_panel._db`, و…)، فقط این‌بار توی مهم‌ترین تابع اتصال کل فایل. برای یه ربات که قراره پول واقعی جابه‌جا کنه، سکوت روی دادهٔ اشتباه خیلی خطرناک‌تر از یه خطای بلند بالاست.

رفع: زیر Postgres، استثنای اصلی دوباره پرتاب می‌شه (نه fallback خاموش)؛ fallback واقعی فقط زیر SQLite (وقتی خودِ دیالوگ SQLite است) معنی داره و دست‌نخورده موند. تأیید شد `bot.py:_db_conn()` و `admin_panel.py:_db()` (که هر دو در همین نشست، بخش ۴۶، رفع شده بودن) از قبل این الگوی خطرناک رو نداشتن — فقط `db.py` این fallback رو داشت.

**تست:** شبیه‌سازی مستقیم قطعی Postgres (`DATABASE_URL` با پسورد اشتباه) — قبل از رفع، بی‌صدا یه فایل `/tmp/...db` فانتوم می‌ساخت؛ بعد از رفع، استثنای واقعی (`OperationalError: password authentication failed`) بالا می‌ره، هیچ فایل فانتومی ساخته نمی‌شه.

### ممیزی تکمیلی مسیرهای مشتری‌محور — بدون یافتهٔ تازه
جست‌وجوی هدفمند برای همون کلاس‌های باگی که این نشست پیدا شدن، این‌بار روی `bot.py`/`api.py`/`core/`/`db.py` (نه فقط `admin_panel.py`):
- `sqlite3.connect` خام باقی‌مانده: فقط همون یکی بالا (رفع شد) + مسیرهای عمداً SQLite-only که قبلاً دیالوگ‌آگاه گارد شدن (`stbak_engine.py`'s SQLite-specific paths, ANALYZE هفتگی).
- `GROUP BY`: همهٔ مواردِ باقی‌مانده در `bot.py`/`db.py`/`iphone_valuation/db.py` بررسی و تک‌تک تأیید شدن — یا از قبل با aggregate/تکرار عبارت درستن، یا روی ستون PRIMARY KEY واقعی (`products.id`, `users.user_id`) گروه‌بندی می‌کنن که طبق قانون functional dependency پستگرس مجازه (دقیقاً همون الگوی تأییدشدهٔ بخش ۴۷).
- `CAST(...AS TEXT)` (الگوی باگ بخش ۴۷)، `INSERT OR REPLACE`/`SELECT changes()` مهاجرت‌نشده، `COLLATE NOCASE` بدون جایگزین: هیچ‌کدوم باقی نمونده.

فایل‌های تغییرکرده: `db.py`.

---

## ۵۱. رفع بحرانی — `BEGIN IMMEDIATE` روی مسیر خرید کاملاً از کار افتاده بود روی Postgres (۲۰۲۶-۰۸-۰۷)

بلافاصله بعد از بخش ۵۰، مالک پروژه با تلاش خرید واقعی محصول این خطا رو گرفت: `syntax error at or near "IMMEDIATE"` — `BEGIN IMMEDIATE;`. ⚠️ این یعنی **قبل از این رفع، هیچ خریدی (نه با کیف‌پول نه بقیهٔ مسیرها) اصلاً روی Postgres کار نمی‌کرد** — دقیقاً همون سناریویی که بخش ۵۰ قرار بود جلوش رو بگیره ولی چون کرالر قبلی فقط GET پنل ادمین رو پوشش می‌داد (نه خودِ فلوی خرید بات)، این باگ تا الان کشف نشده بود.

### ریشهٔ باگ
`BEGIN IMMEDIATE;` سینتکس اختصاصی SQLite است (قفل نوشتنِ *کل فایل* دیتابیس رو فوراً می‌گیره، نه فقط روی commit). Postgres این کلمه رو اصلاً نمی‌شناسه. شش تابع اتمیک پروژه دقیقاً به همین الگو تکیه داشتن: `subtract_wallet_balance` (تنها نقطهٔ کسر کیف‌پول اصلی، صدا زده‌شده از هر مسیر خرید — بخش ۱ سند مینی‌اپ)، `claim_next_feed_item` (تحویل آیتم فید)، `claim_daily_checkin`، `exchange_order`، و کال‌بک تأیید پرداخت در `payment_service.py`.

### رفع دوسطحی — نه فقط رفع سینتکس، بلکه حفظ تضمین همزمانی
۱) **`db_dialect.py`**: `BEGIN IMMEDIATE;` → `BEGIN;` (رفع فوری خطای سینتکس، برای هر ۶ نقطه یکجا).

۲) **مهم‌تر — تضمین قفل ردیف:** روی SQLite، `BEGIN IMMEDIATE` قفل *کل دیتابیس* رو می‌گرفت، پس SELECT اولیهٔ هرکدوم از این توابع به‌خودی‌خود در برابر race condition ایمن بود (هیچ تراکنش دیگه‌ای حتی نمی‌تونست شروع بشه). `BEGIN` سادهٔ Postgres هیچ قفلی از قبل نمی‌گیره — بدون اقدام اضافه، دو تراکنش هم‌زمان می‌تونستن هر دو همون مقدار قدیمی رو بخونن و هر دو موفق به آپدیت بشن: کسر دوبل کیف‌پول (دبل‌تپ دکمهٔ خرید)، claim دوبارهٔ همون آیتم فید، یا — بدتر از همه — پردازش دوبارهٔ یک پرداخت واقعی اگه کال‌بک درگاه دوبار برسه. رفع: تابع تازهٔ `db._row_lock_suffix()` (`"FOR UPDATE "` زیر Postgres، رشتهٔ خالی زیر SQLite که این سینتکس رو اصلاً نداره) به SELECT اولیهٔ هرکدوم از این توابع اضافه شد — قفل ردیف Postgres، معادل دقیق هدف اصلی `BEGIN IMMEDIATE` ولی به‌جای کل دیتابیس فقط همون ردیف (حتی بهتر از نظر همزمانی، چون خریدهای کاربرهای دیگه بلاک نمی‌شن).

`claim_next_feed_item` از قبل یه لایهٔ دفاعی دوم هم داشت (UPDATE شرطی `WHERE id=? AND delivered=0` + چک `rowcount`) که حتی بدون این رفع هم از خرابی دادهٔ واقعی جلوگیری می‌کرد؛ `FOR UPDATE` برای این تابع دفاع در عمقه، نه تنها خط دفاع.

⚠️ **`backup_tools.py:full_reset_database()`** هم همین الگوی `BEGIN IMMEDIATE` رو داره ولی عمداً دست‌نخورده موند — ابزار قدیمی/موازیِ «Robuser» با رشته‌های فارسی mojibake (از قبل در بخش ۲ سند به‌عنوان کد لگاسی مستند شده)، مستقل از `db_conn` (raw `sqlite3.connect`) و جایگزین‌شده با فیچر جدیدتر ریست پنل (بخش ۴۸). رفعش نیاز به بازنویسی دیالوگ‌آگاه جدا داره — فعلاً چون در مسیر خرید/فلوی گزارش‌شده نیست، خارج از این رفع فوری.

### تست انجام‌شده
هارنس مستقیم روی Postgres 16 واقعی: بازتولید دقیق خطای گزارش‌شده (قبل از رفع) و تأیید رفع کامل (بعد از رفع) برای `subtract_wallet_balance`/`claim_next_feed_item`. **تست استرس همزمانی واقعی** (نه فقط تست تک‌رشته‌ای): ۲۰ ترد هم‌زمان روی `subtract_wallet_balance` (موجودی ۱۰۰٬۰۰۰، هرکدوم کسر ۱۰٬۰۰۰) → دقیقاً ۱۰ موفق، موجودی نهایی صفر، بدون کسر دوبل؛ ۱۵ ترد هم‌زمان روی `claim_next_feed_item` (۱۰ آیتم موجود) → دقیقاً ۱۰ claim موفق با شناسهٔ یکتا، بدون تحویل دوبل. `claim_daily_checkin` (idempotency تک‌کاربره) هم تأیید شد. رگرسیون کامل SQLite (هر سه تابع) + کرالر ۶۵ مسیر پنل هم دوباره اجرا شد — بدون تغییر رفتار.

فایل‌های تغییرکرده: `db.py`، `db_dialect.py`، `payment_service.py`.

---

## ۵۲. ممیزی جامع نهایی — `rowid` (خاص SQLite) + `LIKE` حساس به بزرگ/کوچیک روی Postgres (۲۰۲۶-۰۸-۰۷)

مالک پروژه بعد از تأیید رفع بخش ۵۱ («دیگه جایی مشکل ندارم؟») خواست یه بررسی کامل نهایی قبل از معرفی ربات به مشتری واقعی انجام بشه. به‌جای تکیه به همون کلاس باگ‌های قبلی، این‌بار جست‌وجوی سیستماتیک برای **هر** الگوی شناخته‌شدهٔ ناسازگاری SQLite/Postgres در کل کدبیس انجام شد (نه فقط جاهایی که قبلاً خطا داده بودن):

### پیدا شد و رفع شد
1. **`rowid` (خاص SQLite)** — `admin_panel.py:feed_clear` (دکمهٔ «پاک‌سازی آیتم‌های تحویل‌شده» صفحهٔ موجودی) از ستون ضمنی `rowid` برای حذف دسته‌ای (batch) استفاده می‌کرد: `DELETE FROM product_feed WHERE rowid IN (SELECT rowid ... LIMIT 500)`. Postgres اصلاً چنین ستونی نداره — `column "rowid" does not exist`. رفع: جایگزینی با `id` (همون ستون واقعی PRIMARY KEY که `rowid` روی SQLite بهش alias می‌شه، پس رفتار SQLite هم عوض نشد).
2. **`LIKE` حساس به بزرگ/کوچیک روی Postgres** — روی SQLite، `LIKE` برای حروف ASCII پیش‌فرض حساس به بزرگ/کوچیک *نیست*؛ روی Postgres *هست* (باید `ILIKE` باشه). چون خیلی از عنوان محصولات/یوزرنیم‌ها/جزئیات لاتین دارن (اپل آیدی، iPhone، یوزرنیم تلگرام، ...)، این یعنی جست‌وجو روی Postgres بی‌صدا نتیجهٔ کمتر/نادرست می‌داد (نه کرش — یه تخریب خاموش UX که راحت از قلم می‌افته). ۶ نقطه پیدا شد، همه با `LOWER(col) LIKE LOWER(?)` (پرتابل بین هر دو دیالوگ، به‌جای `ILIKE` که SQLite نداره) رفع شدن: `core/products.py:list_products` (جست‌وجوی محصول مینی‌اپ/API — مهم‌ترین مورد چون کاملاً مشتری‌محوره)، `db.py:list_partners`/`get_tutorials`، `admin_panel.py` صفحات محصولات/سفارش‌ها/کاربران/لاگ‌ها.

### بررسی شد، مشکلی نبود
`sqlite_master` (فقط در مسیرهای عمداً SQLite-only)، `PRAGMA` (همون)، `GLOB`/`INSTR`/`printf` (هیچ‌کدوم استفاده نشده)، `json_extract`/توابع JSON SQLite (استفاده نشده)، `ATTACH DATABASE` (استفاده نشده)، `REGEXP` (استفاده نشده)، `WITHOUT ROWID` (استفاده نشده)، ستون‌های `BOOLEAN` بومی (پروژه همیشه از `INTEGER` ۰/۱ استفاده می‌کنه، سازگار با هر دو دیالوگ)، `strftime`/توابع تاریخ خام (کامل تحت پوشش ترجمهٔ `db_dialect.py` از قبل)، بقیهٔ `BEGIN IMMEDIATE` (همه از قبل در بخش ۵۱ رفع شدن؛ `backup_tools.py` عمداً دست‌نخورده، طبق همون توضیح بخش ۵۱).

### تست انجام‌شده
هر دو رفع مستقیم روی Postgres 16 واقعی تست شد: جست‌وجوی محصول با `"iphone"`/`"IPHONE"`/`"IpHoNe"` (هر سه نتیجهٔ یکسان)، پاک‌سازی دسته‌ای آیتم‌های تحویل‌شده (تأیید حذف کامل بدون خطا). کرالر ۶۵ مسیر پنل + تست مستقیم ۶ صفحهٔ جست‌وجو (`?q=...`) با Postgres واقعی + رگرسیون SQLite (تأیید بدون تغییر رفتار) — همه سبز.

فایل‌های تغییرکرده: `admin_panel.py`، `db.py`، `core/products.py`.

---

## ۵۳. مهاجرت lazy کارشناسی آیفون به startup — رفع ۵۰۰ تصادفی روی اولین کلیک بعد از ری‌استارت (۲۰۲۶-۰۸-۰۷)

مالک پروژه گزارش داد بعد از افزودن یه سری تازه در «گروه‌بندی نسل‌ها»ی کارشناسی آیفون، یه‌بار `Internal Server Error` گرفته، دوباره زده و درست شده — و خواست دلیلش رو بدونه، به‌علاوهٔ تأیید این‌که این باعث نشده یه جدول جدید توی SQLite ساخته بشه (نگرانی مشروع، با توجه به باگ فال‌بک خاموش SQLite که در بخش ۵۰ همین امروز رفع شده بود).

### ریشهٔ واقعی (تأیید‌شده با کد، نه حدس)
`iphone_valuation.db.ensure_schema()` — تابع مرکزی مهاجرت/seed این ماژول (۱۴ `CREATE TABLE`، چند بلوک seed اولیه، و ۱۴ تابع `_migrate_*` پشت سر هم) — **هیچ‌جای دیگه‌ای صدا زده نمی‌شد جز lazy، در اولین فراخوانی واقعی هر تابعی از این ماژول** (`list_series`, `create_series`, ...). برخلاف جدول‌های هستهٔ اصلی پروژه (`db.init_db()`, `payment_service.ensure_schema()`) که در `on_startup()` **ایگر** اجرا می‌شن (طبق قاعدهٔ استاندارد بخش ۷)، این ماژول هیچ‌وقت در استارتاپ صدا زده نمی‌شد.

نتیجه: کل زنجیرهٔ سنگین و یک‌بارهٔ migration/seed (که فلگ سراسری `_SCHEMA_DONE` فقط در **انتهای** تابع، بعد از موفقیت کامل، `True` می‌شه) روی **هر پروسهٔ تازه** (بعد از هر ری‌استارت سرویس) صبر می‌کرد تا اولین کلیک واقعی ادمین/کاربر روی این بخش — که این‌بار «افزودن سری تازه» بود، احتمالاً چون سرویس بعد از کار امروز (بخش‌های ۴۸/۴۹، بازطراحی بکاپ + بازیابی بکاپ قدیمی) تازه ری‌استارت شده بود. اگه یه استثنای گذرا (مثلاً یه لحظه فشار اتصال Postgres بلافاصله بعد از ری‌استارت/بازیابی) وسط این زنجیرهٔ ~۴۰ دستور SQL رخ بده، `_SCHEMA_DONE` هیچ‌وقت `True` نمی‌شه و همون درخواست کاربر (نه یه لاگ استارتاپ بی‌ربط) با ۵۰۰ مواجه می‌شه. چون هر تابع `_migrate_*` خودش کامیت مستقل + فلگ ماندگار در `bot_config` داره، تلاش دوم دقیقاً از همون‌جا که مونده بود ادامه داد و بدون خطا کامل شد — این دقیقاً همون رفتار «بار اول خطا، بار دوم بدون هیچ تغییری درست شد» رو توضیح می‌ده.

⚠️ تلاش برای بازتولید دقیق روی یه Postgres واقعی (با پاک‌کردن کامل جدول‌های `iv_*` + فلگ‌های `IV_*` در `bot_config`، دقیقاً شبیه‌سازی «اولین اجرا») **موفق شد بدون خطا** — یعنی خودِ استثنای گذرای واقعی روی سرور تولید قابل‌بازتولید نبود (به همین دلیل هم «گذرا» نامیده می‌شه: به تایمینگ/بار لحظه‌ای شبکه/دیتابیس بستگی داره، نه یه باگ قطعی در منطق مهاجرت). ولی خودِ الگوی مشکل («یه زنجیرهٔ مهاجرت سنگین که فقط با کلیک تصادفی کاربر اجرا می‌شه») مستقل از علت دقیق استثنا، یه ریسک واقعیه که با انتقال به startup برطرف می‌شه.

### تأیید — هیچ جدول SQLite ساخته نشد
`iphone_valuation/db.py` هیچ‌جا مستقیم `sqlite3.connect` نمی‌زنه (`_conn()` همیشه از `db._get_connection()` عبور می‌کنه) — و طبق رفع بخش ۵۰ (همین امروز)، `db._get_connection()` زیر Postgres دیگه هیچ‌وقت بی‌صدا fallback نمی‌کنه، استثنا رو دوباره پرتاب می‌کنه. یعنی همون ۵۰۰ی که کاربر دید **خودش مدرک** اینه که مسیر Postgres واقعاً امتحان و واقعاً fail شده — نه این‌که بی‌صدا موفق شده باشه روی یه فایل SQLite فانتوم. تأیید مستقیم هم انجام شد: هیچ فایل SQLite تازه‌ای در مسیر `DB_PATH` واقعی ایجاد نمی‌شه، صرف‌نظر از موفقیت/شکست این تابع.

### رفع
`payment_service.on_startup()` حالا بلافاصله بعد از `ensure_schema()` خودش، `iphone_valuation.db.ensure_schema()` رو هم صدا می‌زنه (در `try/except` جدا، با لاگ کامل روی شکست — دقیقاً الگوی mount کردن `api.py`: نبود/شکست این بخش کل اپ رو نمی‌شکنه). این باعث می‌شه:
- زنجیرهٔ سنگین مهاجرت/seed یه‌بار، در لحظهٔ استارتاپ سرویس (جایی که تأخیر/خطای احتمالی قابل‌لاگ و منتظره)، اجرا بشه — نه وسط یه کلیک تصادفی ادمین ماه‌ها بعد از آخرین ری‌استارت.
- اگه بازم یه استثنای گذرا رخ بده، حداقل در لاگ استارتاپ (`journalctl`) با پیام واضح «iphone_valuation.db.ensure_schema() در استارتاپ شکست خورد» ثبت می‌شه، به‌جای این‌که فقط به‌عنوان یه ۵۰۰ گنگ به ادمین برسه؛ و چون خودِ `ensure_schema()` idempotent با فلگ per-migration است، اولین فراخوانی بعدی (چه در همون استارتاپ چه با کلیک ادمین) هرچی مونده رو کامل می‌کنه.

**تست انجام‌شده:** روی Postgres 16 واقعی — پاک‌سازی کامل جدول‌های `iv_*` + فلگ‌های `bot_config.IV_*` (شبیه‌سازی «اولین اجرا بعد از ری‌استارت»)، فراخوانی مستقیم `payment_service.on_startup()` (تأیید تکمیل بدون استثنا + seed کامل ۱۶ سری)، سپس `create_series()` بلافاصله بعدش (بدون هیچ تأخیر/کار اضافه‌ای، چون schema از قبل توسط startup آماده شده). رگرسیون کامل SQLite (`DB_PATH` موقت، بدون `DB_DIALECT`) هم با همین توالی تست شد — بدون تغییر رفتار. تأیید مستقل: `db_conn.get_connection()` زیر Postgres واقعاً `_PgConnection` برمی‌گردونه (نه هیچ فایل SQLite). `python3.12 -m py_compile` روی هر فایل تغییرکرده بدون خطا.

فایل‌های تغییرکرده: `payment_service.py`.

---

## ۵۴. گردونهٔ شانس (Wheel of Fortune) — سیستم پاداش عمومی مینی‌اپ، Production-Grade (۲۰۲۶-۰۸-۰۷)

طبق درخواست مالک پروژه («می‌خوام برای بخش مینی‌اپ/PWA یه بخش گردونهٔ امتیاز درست کنم، مثل ناتکوین، که جایزه به کیف‌پول اصلی بره؛ کاملاً بدون Hard Code، همه‌چیز از پنل، معماری آینده‌نگر، کدنویسی تمیز/ماژولار»)، یه سیستم کامل و مستقل ساخته شد — **نه با ساخت زیرساخت موازی، بلکه با گسترش موتور موجود `discount_codes`** (طبق تأیید صریح مالک پروژه). معماری کامل زیر برای هر کار آیندهٔ روی این بخش کافیه؛ نیازی به خوندن دوبارهٔ کد نیست.

### معماری لایه‌ای
```
پنل ادمین (/admin/wheel/*) ──┐
                              ├─→ db.py (wheel_campaigns/wheel_prizes/wheel_spins/wheel_daily_usage)
مینی‌اپ (app.js: openWheel) ─┤        + discount_codes گسترش‌یافته (owner_user_id/source)
   GET  /api/v1/wheel/state ─┤
   POST /api/v1/wheel/spin ──┼─→ core/wheel.py (get_state/spin) ─→ db.issue_personal_discount_code
   GET  /api/v1/me/prizes ───┘                                   یا core/wallet.credit
```
الگوی استاندارد پروژه (بخش ۲۱ سند): منطق خالص در `core/`، روتر نازک در `api.py` (فقط auth+validation)، UI در `app.js`/`app.css`. **`core/wheel.py`** تنها جایی‌ست که تصمیم می‌گیره کدوم جایزه برنده می‌شه — کاملاً سمت سرور، بدون هیچ ورودی کلاینتی مؤثر بر نتیجه.

### دیتابیس (`db.py`, تابع `ensure_wheel_schema`)
- **`wheel_campaigns`** — `title, starts_at, ends_at, active, daily_spin_limit (نال=fallback به تنظیمات کلی), theme_json, sort_order`. **همیشه فقط یک کمپین فعال** — تضمین‌شده در سطح دیتابیس با `set_active_wheel_campaign()` (یه تراکنش: همه رو غیرفعال، بعد فقط یکی رو فعال)، نه با تکیه به انضباط ادمین.
- **`wheel_prizes`** — `campaign_id, title, icon, color, prize_type, value, max_discount_value, weight (احتمال، نسبی نه درصد مطلق), total_limit (سقف عمری سخت‌گیرانه), daily_limit (سقف روزانه نرم), issued_count, validity_hours, active, sort_order, description`. شش نوع جایزه (`WHEEL_PRIZE_TYPES`): `wallet_credit`, `discount_percent`, `discount_fixed`, `extra_spin`, `no_win`, `physical_gift` (پیاده‌سازی کامل هدیهٔ فیزیکی — پیگیری/ارسال دستی — عمداً فاز آینده، فقط لاگ می‌شه).
- **`wheel_spins`** — لاگ کامل هر چرخش (**هم تاریخچهٔ پنل هم تنها منبع آمار** — یک جدول، دو مصرف): `user_id, campaign_id, prize_id, prize_type, prize_title (denormalized — حتی اگه جایزه بعداً حذف/ویرایش بشه، تاریخچه دقیق می‌مونه), amount, discount_code, discount_code_id, status, ip, device_fingerprint, session_id`.
- **`wheel_daily_usage`** — شمارندهٔ اتمیک محدودیت روزانه (`user_id, campaign_id, usage_date, spins_used`) — چرا جدول جدا از `wheel_spins`: چون `COUNT(*)` روی یه مجموعه قفل‌پذیر نیست؛ این جدول دقیقاً الگوی `daily_checkins` (یک ردیف per user+day که با `SELECT...FOR UPDATE` قفل می‌شه) رو تکرار می‌کنه.
- **تنظیمات کلی** جدول جدا نداره — طبق الگوی موجود پروژه (`get_cfg_json`/`set_cfg`) به‌صورت JSON زیر کلید `bot_config.WHEEL_SETTINGS` ذخیره می‌شه (`get_wheel_settings`/`save_wheel_settings`).
- **seed اولیه**: فقط روی نصب کاملاً تازه (کمپین پیش‌فرض خالی)، ۱ کمپین + ۶ جایزهٔ نمونه — صرفاً داده (مثل seed `iv_coefficients`)، نه منطق هاردکد؛ همه از پنل قابل ویرایش/حذف.
- **ایگر در startup، نه lazy**: `payment_service.on_startup()` مستقیم `db.ensure_wheel_schema()` رو صدا می‌زنه — طبق درسی که همین امروز از بخش ۵۳ (باگ ۵۰۰ تصادفی مهاجرت lazy کارشناسی آیفون) گرفته شد؛ همون کلاس ریسک این‌جا از ابتدا پیشگیری شد.

### گسترش `discount_codes` (نه سیستم موازی)
سه ستون تازه: `owner_user_id` (نال‌پذیر — نال=کد عمومی معمولی، پرشده=کد **شخصی**، فقط همون کاربر می‌تونه استفاده کنه؛ چک در `validate_discount`)، `source` (`'winback'|'wheel'|...`)، `source_ref_id`. تابع عمومی تازهٔ **`db.issue_personal_discount_code(user_id, disc_type, value, expire_hours, max_value, description, source, source_ref_id)`** تک‌منبع حقیقت صدور کد شخصیه — هم گردونه هم `create_winback_code` (ریفکتور شد تا از همین تابع استفاده کنه، به‌جای منطق تکراری) ازش استفاده می‌کنن؛ هر مکانیزم پاداشی آینده (معرفی، تولد، ...) هم فقط کافیه همینو صدا بزنه. **`list_user_personal_codes(user_id)`** پایهٔ صفحهٔ «جوایز من» — همهٔ کدهای شخصی کاربر از هر منبعی، با وضعیت زندهٔ محاسبه‌شده (`active`/`used`/`expired`/`inactive`).

⚠️ **باگ واقعی پیدا و رفع‌شده حین ساخت این فیچر (نه فرضی):** `validate_discount`/کد جدید `list_user_personal_codes` مقدار «الان» رو با `datetime.utcnow().isoformat()` (فرمت `...THH:MM:SS.ffffff`) با `expires_at` ذخیره‌شده توسط `datetime('now','localtime')` (فرمت `...  HH:MM:SS`، جداکنندهٔ فاصله) مقایسه می‌کردن. چون در مقایسهٔ لغوی رشته `' '(0x20) < 'T'(0x54)` همیشه برقراره، **هر کدی که انقضاش همون روز باشه** (دقیقاً سناریوی رایج جایزهٔ گردونه با اعتبار ۱ تا ۲۴ ساعته) همیشه اشتباهاً «منقضی‌شده» تشخیص داده می‌شد — فقط با تاریخ فردا/بعد قابل مشاهده نبود چون رقم روز تفاوت رو جبران می‌کرد. این یه باگ preexisting در `validate_discount` بود (روی کدهای winback هم اثر داشت)، نه چیزی مخصوص گردونه — رفع شد با `datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')` (فرمت هم‌راستا با مقدار ذخیره‌شده) در هر دو تابع.

### موتور چرخش (`core/wheel.py`)
- **`get_state(user_id)`** — تنظیمات+کمپین فعال+لیست جوایز (با `percent` احتمال فقط اگه `show_odds` روشن باشه)+چرخش باقیمانده امروز؛ برای رندر اولیهٔ UI.
- **`spin(user_id, ip, device_fingerprint, session_id)`** — تنها مسیر صدور جایزه، کاملاً اتمیک:
  1. `db.try_consume_wheel_spin` (قفل ردیف روی `wheel_daily_usage`، دقیقاً الگوی `claim_daily_checkin`) — اگه سقف روزانه پر باشه، همین‌جا رد می‌شه، **قبل از** هر انتخاب/صدور جایزه‌ای.
  2. فیلتر جوایز فعال بر اساس `total_limit`/`daily_limit`، بعد انتخاب وزن‌دار تصادفی (`_weighted_pick`) با یه حلقهٔ retry: اگه جایزهٔ انتخاب‌شده `total_limit` داشته باشه، `db.try_claim_wheel_prize_slot` (افزایش اتمیک `issued_count` با `WHERE issued_count<total_limit`+چک `rowcount`، دقیقاً الگوی `claim_next_feed_item`) رو امتحان می‌کنه؛ اگه شکست خورد (باخت race یه جایزهٔ محدود)، از pool حذف و دوباره انتخاب می‌کنه. اگه pool کامل خالی بشه (edge-case آماری تقریباً غیرممکن)، یه جایزهٔ fallback «بدون برد» بدون id واقعی برمی‌گرده تا کاربر هیچ‌وقت پاسخ خالی/۵۰۰ نگیره.
  3. صدور واقعی (`_issue_prize`): `wallet_credit`→`core.wallet.credit`؛ `discount_percent/fixed`→`db.issue_personal_discount_code`؛ `extra_spin`→`db.grant_extra_wheel_spins` (مصرف امروز رو کم می‌کنه، بلافاصله چرخش بعدی ممکن می‌شه)؛ `no_win`/`physical_gift`→فقط لاگ.
  4. `db.insert_wheel_spin` — لاگ کامل، همیشه، صرف‌نظر از نوع جایزه.
- **ضدتقلب:** `ip`/`device_fingerprint`/`session_id` در هر چرخش ثبت می‌شن (لاگ، نه ورودی تصمیم‌گیری) — `device_fingerprint` سمت کلاینت با یه شناسهٔ تصادفیِ ذخیره‌شده در `localStorage` تولید می‌شه (best-effort، نه یه مکانیزم امنیتی سخت‌گیر؛ لایهٔ واقعی امنیت همون auth تلگرام initData + قفل اتمیک روزانه‌ست).
- **تست همزمانی مستقیم روی Postgres 16 واقعی:** ۲۰ چرخش هم‌زمان همون کاربر با `daily_limit=1` → دقیقاً ۱ موفق، ۱ ردیف در `wheel_spins`؛ ۱۰ تلاش claim روی جایزهٔ `total_limit=3` → دقیقاً ۳ موفق.

### API (`api.py`, زیر `/api/v1`)
`GET /wheel/state` (auth اختیاری — کاربر مهمان هم گردونه رو می‌بینه، فقط `spins_remaining` نداره)، `POST /wheel/spin` (auth اجباری)، `GET /me/prizes` (auth اجباری). هر سه تست شدن end-to-end با `TestClient` روی Postgres واقعی.

### پنل ادمین (`/admin/wheel/*`، permission تازهٔ `wheel` با `PERM_LEGACY→growth`)
- **`/admin/wheel`** — داشبورد: KPI (چرخش امروز، کاربران یکتا، نرخ برد، جمع پرداختی کیف‌پول، کد تخفیف صادرشده/نرخ استفاده)، نمودار روزانهٔ ۳۰ روز (Chart.js، همون الگوی داشبورد اصلی)، لیست کمپین‌ها/محبوب‌ترین جوایز، لینک به زیرصفحات.
- **`/admin/wheel/settings`** — همهٔ آیتم‌های خواسته‌شده (فعال/غیرفعال، عنوان/توضیحات/بنر/رنگ/تم، محدودیت روزانه پیش‌فرض، ساعت ریست، مدت انیمیشن/نمایش نتیجه، صدا/لرزش/نمایش احتمال) — یک فرم، یک دکمهٔ ذخیره (الگوی bulk-save استاندارد پروژه، بخش ۲۲.۱۱).
- **`/admin/wheel/campaigns`** — لیست bulk-save + فرم افزودن + دکمهٔ فعال‌سازی (`set_active_wheel_campaign`) + حذف (کسکید روی جوایز خودش، `wheel_spins` دست‌نخورده می‌مونه چون denormalized).
- **`/admin/wheel/prizes?campaign_id=`** — لیست bulk-save scoped به یه کمپین (دراپ‌داون انتخاب کمپین)، هر ردیف تمام فیلدهای جایزه (نوع/مقدار/سقف تخفیف/اعتبار/وزن/سقف کل/سقف روزانه/اولویت/فعال/توضیحات) + نمایش زندهٔ درصد احتمال محاسبه‌شده از مجموع وزن‌ها + فرم افزودن.
- **`/admin/wheel/history`** — جست‌وجو (کاربر/نوع جایزه/کمپین) + صفحه‌بندی، هر ردیف کاربر+جایزه+نوع+مبلغ+کد تخفیف+زمان.
- همهٔ ۵ صفحه + همهٔ POSTها با `TestClient` روی Postgres 16 واقعی تست شدن (رندر، bulk-save، افزودن، فعال‌سازی، حذف) و رگرسیون کامل روی SQLite هم تکرار شد.

### UI مینی‌اپ (`app.js`/`app.css`/`index.html`)
- کارت ورودی «🎡 گردونهٔ شانس» در تب «ابزار مفید» (اولین کارت، فعال — نه «به‌زودی»)؛ ردیف «🎁 جوایز من» در تب حساب.
- **چرخ‌گردون** — دیسک با `conic-gradient` (رنگ هر سگمنت از `prize.color`)، لیبل‌های رادیال (آیکون+٪ اختیاری) با ترفند استاندارد CSS (`rotate(mid) + offset ثابت + rotate(-mid)` برای نگه‌داشتن متن عمودی)، هاب مرکزی دکمهٔ چرخش (جدا از دیسک، نمی‌چرخه)، نشانگر ثابت بالا. چرخش با `transform:rotate()` + `transition` (مدت/easing از تنظیمات پنل) — زاویهٔ هدف از `res.prize.id` (نه از حدس کلاینت) محاسبه می‌شه؛ چند دور اضافه + جیتر کوچیک داخل سگمنت برای طبیعی‌بودن.
- **افکت‌ها، بدون وابستگی خارجی/فایل باینری/CDN تازه:** صدا با Web Audio API (تن‌های تولیدشده، نه فایل mp3)، لرزش با `Telegram.WebApp.HapticFeedback` (همون الگوی موجود چک‌این)، کانفتی با یه canvas سبک دستی (بدون کتابخونه) — هر سه پشت تنظیمات پنل (`sound_enabled`/`haptic_enabled`/`animation_enabled`) قابل خاموش‌کردن.
- کارت نتیجه: آیکون/عنوان/توضیح جایزه + (اگه کد تخفیف بود) جعبهٔ کد با دکمهٔ کپی + تاریخ انقضا + دکمهٔ «دیدن در جوایز من». موجودی کیف‌پول تب حساب (`#me-bal`) بلافاصله بعد از جایزهٔ نقدی به‌روز می‌شه، بدون نیاز به رفرش.
- صفحهٔ «جوایز من» — لیست کدهای شخصی با نشان وضعیت رنگی + دکمهٔ کپی برای کدهای فعال.
- ⚠️ **تست لایوت با Playwright روی یه صفحهٔ ایزولهٔ مینیمال انجام شد** (الگوی مستندشدهٔ بخش ۲۱ سند — vendor واقعی Framework7 در این سندباکس قابل‌دانلود نبود، شبکهٔ محدود): تأیید شد هر ۶ لیبل جایزه دقیقاً روی شعاع ۹۲px از مرکز دیسک (شعاع ۱۳۰px) می‌شینن — بدون سرریز؛ هاب مرکزی دقیقاً هم‌مرکز با دیسکه (offset صفر)؛ بدون سرریز افقی صفحه، هم روشن هم تاریک. قبل از نتیجه‌گیری از این تست، الگوی سه باگ لایوت قبلی پروژه (بخش ۲۱) صریحاً بررسی شد: نه `width:100%`+padding بدون `box-sizing:border-box` وجود داره (چون هیچ‌جا `width:100%` صریح استفاده نشده — عناصر block/flex-stretch پیش‌فرض خودشون این مشکل رو ندارن)، نه `align-self` استفاده شده.

### تست جامع پایانی
سناریوی end-to-end کامل (Postgres 16 واقعی + رگرسیون SQLite): ساخت اسکیما+seed → فعال‌سازی تنظیمات → چرخش موفق → مصرف سقف روزانه → چرخش دوم رد می‌شه → کد شخصی صادرشده در «جوایز من» با وضعیت `active` (نه `expired` کاذب) → `validate_discount` توسط مالک موفق، توسط کاربر دیگه رد (پیام «این کد مخصوص شماست») → ۲۰ چرخش هم‌زمان روی `daily_limit=1` دقیقاً ۱ موفق → ۱۰ claim هم‌زمان روی جایزهٔ `total_limit=3` دقیقاً ۳ موفق → هر ۵ صفحهٔ پنل + همهٔ عملیات POST (افزودن/bulk-save/فعال‌سازی/حذف کمپین و جایزه) روی هر دو دیالوگ.

### نقاط باز/فاز آینده (طبق بخش «قابلیت توسعه» درخواست مالک پروژه)
