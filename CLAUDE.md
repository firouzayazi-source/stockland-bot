...
قیمت هر گوشی بسته به پارت نامبر (LL/A, ZA/A, CH/A) **و** رنگ می‌تونه فرق کنه، پس `iv_capacities` دیگه یکتا-به-ازای-(مدل+ظرفیت) نیست — یکتا-به-ازای-(مدل+ظرفیت+`part_number`+`color`) هست؛ `part_number=''`/`color=''` یعنی «قیمت عمومی بدون پارت/رنگ مشخص». ⚠️ **تصمیم اولیهٔ این نشست این بود که رنگ فقط توصیفیه و روی قیمت اثر نداره — اشتباه بود و مالک پروژه صریحاً تصحیحش کرد؛ رنگ هم دقیقاً مثل پارت یک بعد قیمت‌گذاریه.**

`ivdb.resolve_capacity(model_id, capacity_label, part_number='', color='')` قیمت رو با ۴ سطح fallback به ترتیب دقت پیدا می‌کنه: (پارت دقیق+رنگ دقیق) → (پارت دقیق+رنگ عمومی) → (پارت عمومی+رنگ دقیق) → (پارت عمومی+رنگ عمومی) — یعنی ادمین فقط جایی که قیمت واقعاً فرق داره لازمه پارت/رنگ مشخص وارد کنه، بقیه از قیمت عمومی‌تر استفاده می‌کنن. `ivdb.upsert_capacity(...)` (نه `create_capacity` مستقیم از فرم پنل): اگه ترکیب (مدل+ظرفیت+پارت+رنگ) از قبل ثبت شده باشه آپدیت می‌کنه، وگرنه می‌سازه — **هیچ‌وقت خطای «قبلاً ثبت شده» رد نمی‌شه** (این پیام دقیقاً شبیه باگ قدیمی «ذخیره نمی‌شه» به نظر می‌رسید، پس عمداً حذف شد). `ivdb.get_capacity_exact(model_id, label, part, color)` تطبیق دقیق (نه fallback) رو برمی‌گردونه — پایهٔ upsert. `ivdb.list_capacity_labels(model_id)` برچسب‌های یکتای ظرفیت (صرف‌نظر از پارت/رنگ، مرتب‌شده با `capacity_sort_key` به ترتیب اندازهٔ واقعی نه الفبایی) رو برمی‌گردونه.

**پارت فقط برای مدل‌های `dual_sim_parts`-دار پرسیده/پیکربندی می‌شه** (از iPhone XS Max به بعد؛ iPhone XS و پایین‌تر چون `dual_sim_parts` خالیه، اصلاً پرسیده نمی‌شه — همون فلگ سیاست سیم‌کارت موجود `_iv_sim_policy`). **رنگ برعکس، همیشه پرسیده/قابل‌تنظیمه** (مستقل از `dual_sim_parts`) — چون همهٔ مدل‌ها رنگ‌های متفاوت دارن، فقط بعضی‌ها پارت‌محورن.
...
**`/admin/iphone/models` (بازطراحی‌شده، ۲۰۲۶-۰۷-۲۵، دور دوم برای رنگ):** هر مدل یه `<details>` جدا. ظرفیت‌ها زیر یک هدر گروه‌بندی‌شده بر اساس برچسب ظرفیت نمایش داده می‌شن؛ هر ردیف قیمت بج پارت (اگه مدل پارت‌محور باشه) + بج رنگ (همیشه) + دکمهٔ حذف داره. **به‌جای فرم‌های پراکندهٔ قبلی** (افزودن ظرفیت جدا، افزودن قیمت پارت دیگه جدا) الان **یک فرم واحد** «📝 ثبت/به‌روزرسانی قیمت این مدل» زیر لیست هست: سه دراپ‌داون — ظرفیت (`ivdb.list_capacity_labels(model_id)`، فقط ظرفیت‌های خودِ این مدل، + «سایر» دستی)، رنگ (`ivdb.list_colors(model_id)`، فقط رنگ‌های خودِ این مدل، همیشه نشون داده می‌شه)، پارت (فقط اگه مدل پارت‌محور باشه) — بعدش باکس‌های قیمت، یک دکمهٔ ذخیره که به `POST /iphone/capacities/upsert` می‌ره (نه `/add`). فرم «+ افزودن رنگ» (مدیریت کاتالوگ رنگ‌های این مدل، منبع دراپ‌داون بالا) جدا مونده، دراپ‌داون رنگ‌های رایج `_IV_COMMON_COLORS` + «سایر». همهٔ فیلدهای قیمت `type="text" inputmode="numeric/decimal"` هستن (نه `type="number"` — بخش ۱۷ رو ببین، دلیلش رو). دکمهٔ «حذف مدل» با `confirm()` کنار toggle فعال/غیرفعال.

### ربات (`bot.py`)
دکمهٔ `MAIN_BTN_IPHONE_VALUATION` در منوی اصلی (`keyboards.main_menu`). ویزارد اینلاین با `user_states[uid]["mode"]="iphone_valuation"` → `"iv_seller_price"` → `"iv_city"` (سه حالت متنی/اینلاین ترکیبی). مراحل (ترتیب از ۲۰۲۶-۰۷-۲۵ چند بار عوض شد): انتخاب مدل → **[فقط اگه `dual_sim_parts` پر باشه] پارت نامبر** → ظرفیت (`ivdb.list_capacity_labels`) → **رنگ** (`ivdb.list_colors(model_id)`، فقط رنگ‌های همین مدل + «سایر») → با انتخاب رنگ، `ivdb.resolve_capacity(model_id, label, part_number, color)` قیمتِ درستِ همون ترکیب رو پیدا می‌کنه (`state["capacity_id"]`) → هشت گزینهٔ شرایط دستگاه (از `_IV_COEFF_STEPS`، به ترتیب `iv_coefficients`) → تست امکانات (بله/خیر) → نوع فروشنده → قیمت پیشنهادی (متن آزاد، «ندارم» = رد شدن) → شهر (متن آزاد، «ندارم» = رد شدن) → `_iv_finalize` که `service.valuate()` رو صدا می‌زنه و پیام نهایی رو می‌سازه (برچسب پارت از `ivdb.PART_OPTIONS`، رنگ هم در پیام نهایی نمایش داده می‌شه).

**چرا ترتیب اینه:** قیمت نهایی هم‌زمان به ظرفیت، پارت، *و* رنگ وابسته‌ست (`resolve_capacity` هر سه رو می‌خواد)، پس همه باید قبل از قطعی‌شدن انتخاب مشخص بشن. برای مدل‌هایی که `dual_sim_parts` خالیه (iPhone XS و پایین‌تر، مینی‌ها) سؤال پارت کلاً پرسیده نمی‌شه چون بی‌معنیه — `state["part_number"]=""` می‌مونه؛ سؤال رنگ برعکس **همیشه** پرسیده می‌شه (مستقل از `dual_sim_parts`)، چون رنگ برای همهٔ مدل‌ها روی قیمت اثر داره.
