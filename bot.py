...
reseller_signup = {}  # uid -> {phone, username, full_name, city}
...
# --- Backup/Restore + Full Reset (added by patch) ---
...
def validate_backup_db(path_: str) -> tuple[bool, str]:
    import sqlite3
    required = {
        "delivery_messages","wallets","products","partners","orders","zarinpal_transactions",
        "product_feed","feed_alert_settings",# "other_services",  # preserved on full reset"ui_texts","tickets","pending_deliveries"
    }
    try:
...
def restore_db_from_backup(backup_path: str) -> str:
    ...
    # Ensure no WAL leftovers; best-effort checkpoint then replace.
    try:
        import sqlite3
    ...
    # Move current db aside (if exists), then move tmp into place
    if os.path.exists(DB_PATH):
        os.replace(DB_PATH, bak)
    os.replace(tmp, DB_PATH)

    # Remove wal/shm to avoid mismatch
    for ext in ("-wal","-shm"):
        p = DB_PATH + ext
        if os.path.exists(p):
            try: os.remove(p)
...
def admin_backup_menu():
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(
        types.InlineKeyboardButton("📤 خروجی/بکاپ (دانلود فایل دیتابیس)", callback_data="admin_export_backup"),
        types.InlineKeyboardButton("📥 بازیابی از بکاپ (آپلود فایل)", callback_data="admin_import_backup"),
...
def admin_full_reset_confirm_menu(step: int = 1):
    ...
    if step == 1:
        kb.add(
            types.InlineKeyboardButton("✅ ادامه", callback_data="admin_full_reset_2"),
            types.InlineKeyboardButton("❌ انصراف", callback_data="admin_settings"),
        )
    else:
        kb.add(
            types.InlineKeyboardButton("🔥 تایید نهایی", callback_data="admin_full_reset_do"),
            types.InlineKeyboardButton("❌ انصراف", callback_data="admin_settings"),
        )
    return kb
...
def full_reset_database():
    ...
    try:
        ...
        # حذف فقط زیر‌دسته‌ها
        # فقط service_key = 'general' باقی بماند
        try:
            conn.execute("""
...
def handle_confirm_full(call):
    ...
    # انتظار: confirm_full_{category}_{pid}
    if len(parts) < 4:
...
def handle_confirm_wallet(call):
    ...
    # انتظار: confirm_wallet_{category}_{pid}
    if len(parts) < 4:
        bot.answer_callback_query(call.id, "داده نامعتبر است", show_alert=True)
        return
    ...
    wallet_reserved = wallet_balance
    gateway_amount = eff_price - wallet_reserved

    # اگر مبلغ درگاه کمتر از حداقل بود
    if gateway_amount < MIN_GATEWAY:
        gateway_amount = MIN_GATEWAY
